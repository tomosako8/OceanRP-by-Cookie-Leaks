...
RegisterCommand('+-slot1', function()	
	...
	if not IsControlPressed(0, Keys['LEFTALT']) and (not IsControlPressed(0, Keys['LEFTSHIFT'])) then
		Select(1)
	end
end)
...
function Select(number)
	if not exports['esx_policejob']:IsCuffed() and exports['ocean_animacje']:PedStatus() and not IsPedInAnyPoliceVehicle(PlayerPedId()) then
