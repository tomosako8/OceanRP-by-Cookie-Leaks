function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, job, name, coords, character, secondjob, thirdjob, dealerLevel, digit, slots, components, status)
	local self = {}
	...
	self.setStatus = function(stat)
		self.status = stat
	end

	self.removeFromStatus = function(name, val)
		for i=1, #self.status, 1 do
			if self.status[i].name == name then
				self.status[i].val = userData.status[i].val - val
				self.status[i].percent = (userData.status[i].val / 1000000) * 100
			end
		end
	end
	...
	self.updateComponents = function(val)
		self.components = val
	end
	...
	self.addInventoryItemSaved = function(item)
		
		if item then

			TriggerEvent('esx:onAddInventoryItem', self.source, item.name, 1)
			self.triggerEvent('esx:addInventoryItem', item, 1)
		end
	end
