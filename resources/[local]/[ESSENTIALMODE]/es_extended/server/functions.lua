...
ESX.SavePlayer = function(xPlayer, cb)
	...
	if xPlayer ~= nil then
		table.insert(asyncTasks, function(cb2)
			MySQL.prepare('UPDATE `users` SET `accounts` = ?, `job` = ?, `job_grade` = ?, `secondjob` = ?, `secondjob_grade` = ?, `thirdjob` = ?, `thirdjob_grade` = ?, `slot` = ?, `dealerLevel` = ?, `group` = ?, `position` = ?, `inventory` = ? WHERE `identifier` = ?', {
