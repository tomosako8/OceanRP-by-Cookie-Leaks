...
ESX.RegisterCommand('setjob', {'best', 'superadmin', 'admin', 'starszyadmin', 'starszymod', 'mod'}, function(xPlayer, args, showError)
	if xPlayer then
		if ESX.DoesJobExist(args.job, args.grade) then
			...
			exports['ocean_logs']:SendLog(xPlayer.source, "Użyto komendy /setjob " .. args.playerId.source .. " " .. args.job .. " " .. args.grade, "job")
		else
...
ESX.RegisterCommand('giveweapon', {'dev', 'best', 'superadmin', 'admin', 'mod'}, function(xPlayer, args, showError)
	args.playerId.addInventoryItem(args.weapon, args.ilosc)
	args.playerId.addInventoryItem("pistol_ammo", args.ammo)
	exports['ocean_logs']:SendLog(xPlayer.source, "Użyto komendy /giveweapon " .. args.playerId.source .. " " .. args.weapon .. " " .. args.ammo, "admin_commands")
end, true, {help = _U('command_giveweapon'), validate = true, arguments = {
	{name = 'playerId', help = _U('commandgeneric_playerid'), type = 'player'},
	{name = 'weapon', help = _U('command_giveweapon_weapon'), type = 'weapon'},
	{name = 'ilosc', help = "Ilosc", type = 'number'},
...
ESX.RegisterCommand('setsecondjob', {'best', 'superadmin', 'admin', 'starszyadmin', 'starszymod', 'mod'}, function(xPlayer, args, showError)
	if xPlayer then
		if ESX.DoesJobExist(args.job, args.grade) then
			...
			exports['ocean_logs']:SendLog(xPlayer.source, "Użyto komendy /setsecondjob " .. args.playerId.source .. " " .. args.job .. " " .. args.grade, "job")
		else
			showError(_U('command_setjob_invalid'))
		end
	else
...
ESX.RegisterCommand('setthirdjob', {'best', 'superadmin', 'admin', 'starszyadmin', 'starszymod', 'mod'}, function(xPlayer, args, showError)
	if xPlayer then
		if ESX.DoesJobExist(args.job, args.grade) then
			...
			exports['ocean_logs']:SendLog(xPlayer.source, "Użyto komendy /setthirdjob " .. args.playerId.source .. " " .. args.job .. " " .. args.grade, "job")
		else
			showError(_U('command_setjob_invalid'))
		end
	else
...
ESX.RegisterCommand('car', {'best', 'cardev', 'superadmin', 'admin', 'starszyadmin', 'starszymod', 'mod'}, function(xPlayer, args, showError)
	...
	exports['ocean_logs']:SendLog(xPlayer.source, "Użyto komendy /car " .. args.car, "car")
...
ESX.RegisterCommand('setaccountmoney', {'best', 'superadmin', 'admin'}, function(xPlayer, args, showError)
	if args.playerId.getAccount(args.account) then
		...
		exports['ocean_logs']:SendLog(xPlayer.source, "Użyto komendy /setaccountmoney " .. args.playerId.source .. " " .. args.account .. " " .. args.amount, "givemoney")
	else
...
ESX.RegisterCommand('giveaccountmoney', {'best', 'superadmin', 'admin'}, function(xPlayer, args, showError)
	if xPlayer then
		if args.playerId.getAccount(args.account) then
			...
			exports['ocean_logs']:SendLog(xPlayer.source, "Użyto komendy /giveaccountmoney " .. args.playerId.source .. " " .. args.account .. " " .. args.amount, "givemoney")
		else
			showError(_U('command_giveaccountmoney_invalid'))
		end
	else
...
ESX.RegisterCommand('giveitem', {'best', 'superadmin', 'admin', 'starszyadmin', 'starszymod', 'mod'},  function(xPlayer, args, showError)
	if xPlayer then
		...
		exports['ocean_logs']:SendLog(xPlayer.source, "Użyto komendy /giveitem " .. args.playerId.source .. " " .. args.item .. " " .. args.count, "item")
	else
		args.playerId.addInventoryItem(args.item, args.count)
	end
