...
function genTxt() 
	...
	CreateRuntimeTextureFromImage(txd, 'logo', 'dependencies/img/ocean2.png')
	CreateRuntimeTextureFromImage(txd, 'banner', 'dependencies/img/ocean1.png')
end
...
function DrawPlayerInfo(target, custom)
	...
	if ply and ply ~= -1 then
		exports["esx_oceanchat"]:showIcon("fas fa-eye", false, 0)
...
function StopDrawPlayerInfo(cb)
	...
	exports["esx_oceanchat"]:hideIcon()
	if drawCustom then
...
RegisterNetEvent("exilerp:rzadowyCalled", function(admin)
...
function GenerateMenu(fresh)
	...
	mainMenu = NativeUI.CreateMenu("", "~b~OceanRP", menuOrientation, 0, "easyadmin", "banner", "logo")
		...
		ESX.TriggerServerCallback('EasyAdmin:players', function(players)
			for _, data in ipairs(players) do
				...
				if permissions.spectate then
					...
					thisPlayer:AddItem(thisItem)
					thisItem.Activated = function(ParentMenu,SelectedItem)
						TriggerServerEvent("exilerp:callRzadowy", data.id)
					end
				end
