...
ESX.RegisterCommand('goto', {'trialsupport', 'support', 'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin'}, function(xPlayer, args, showError)
    if args.id then
        local xTarget = ESX.GetPlayerFromId(args.id)
		if xPlayer and xTarget then
			local targetCoords = xTarget.coords
			xPlayer.setCoords(targetCoords)
			exports['cookie_logs']:SendLog(xPlayer.source, "Użyto komendy /goto do gracza: " .. args.id, "admin_commands2")
        end
    end
...
ESX.RegisterCommand('bring', {'trialsupport', 'support', 'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin'}, function(xPlayer, args, showError)
    if args.id then
        local xTarget = ESX.GetPlayerFromId(args.id)
        if xPlayer and xTarget then
            local targetCoords = xPlayer.coords
			xTarget.setCoords(targetCoords)
			exports['cookie_logs']:SendLog(xPlayer.source, "Użyto komendy /bring na graczu: " .. args.id, "admin_commands2")
        end
    end
...
ESX.RegisterCommand('slap', {'best' , 'superadmin'}, function(xPlayer, args, showError)
    if args.id then
        local xTarget = ESX.GetPlayerFromId(args.id)
        if xPlayer and xTarget then
			local xTarget = ESX.GetPlayerFromId(args.id)
			TriggerClientEvent('EasyAdmin:Slap', xTarget.source)
			exports['cookie_logs']:SendLog(xPlayer.source, "Użyto komendy /slap na graczu: " .. xTarget.source, "admin_commands")
        end
    end
...
ESX.RegisterCommand('slay', {'best', 'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin'}, function(xPlayer, args, showError)
    if args.id then
        local xTarget = ESX.GetPlayerFromId(args.id)
        if xPlayer and xTarget then
			local xTarget = ESX.GetPlayerFromId(args.id)
            TriggerClientEvent('EasyAdmin:Slay', xTarget.source)
			exports['cookie_logs']:SendLog(xPlayer.source, "Użyto komendy /slay na graczu: " .. xTarget.source, "admin_commands")
        end
    end
...
ESX.RegisterCommand('tpp', {'support', 'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin'}, function(xPlayer, args, showError)
    if args.steamid and args.targetid then
        ...
        if xPlayer and xPlayerTarget then
			...
			exports['cookie_logs']:SendLog(xPlayer.source, "Użyto komendy /tpp gracza: " .. args.steamid .. " do gracza: " .. args.targetid, "admin_commands2")
        end
    end
...
ESX.RegisterCommand('savecar', {'admin', 'starszyadmin', 'superadmin', 'best'}, function(xPlayer, args, showError)
	if args.targetid ~= nil then
		...
		if xPlayer and tPlayer then
			...
			exports['cookie_logs']:SendLog(xPlayer.source, "Użyto komendy /savecar " .. args.targetid, "car")
		end
	end
...
ESX.RegisterCommand('updateplayer', {'mod', 'starszymod', 'starszymod', 'admin', 'starszyadmin', 'superadmin'}, function(xPlayer, args, showError)
...
ESX.RegisterCommand('updateplate', {'admin', 'starszyadmin', 'superadmin', 'best'}, function(xPlayer, args, showError)
	if args.oldPlate ~= nil and args.newPlate ~= nil then
		...
		exports['cookie_logs']:SendLog(xPlayer.source, "Użyto komendy /updateplate " .. oldPlate .. " " .. newPlate, "car")
	end
...
ESX.RegisterCommand('delcar', {'superadmin', 'best'}, function(xPlayer, args, showError)
	if args.Plate ~= nil then
		if xPlayer then
			...
			exports['cookie_logs']:SendLog(xPlayer.source, "Użyto komendy /delcar "..Plate, "car")
		else
			local Plate = string.upper(args.Plate)
			MySQL.update('DELETE FROM owned_vehicles WHERE plate = @plate',{ 
				['@plate'] = Plate
...
ESX.RegisterCommand('givecar', {'admin', 'starszyadmin', 'superadmin', 'best'}, function(xPlayer, args, showError)
    if args.steamid and args.targetid then
		local xPlayer = ESX.GetPlayerFromId(args.steamid)
		if xPlayer then
			TriggerClientEvent('esx:spawnVehicle', args.steamid, args.targetid)
			exports['cookie_logs']:SendLog(xPlayer.source, "Użyto komendy /givecar o modelu: " .. args.targetid .. " dla gracza o id: " .. args.steamid, "car")
		end
	end
...
ESX.RegisterCommand('admins', {'trialsupport', 'support', 'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin', 'best'}, function(xPlayerr, args, showError)
...
ESX.RegisterCommand('delchar', {'admin', 'starszyadmin', 'superadmin', 'best'}, function(xPlayer, args, showError)
...
AddEventHandler('EasyAdmin:SaveCar', function(vehicleProps, recieved)
	...
	if recieved == 1337 then
		...
		exports['cookie_logs']:SendLog(_source, "Próbwa wywołania eventu SaveCar", "anticheat")
	end
end)

ESX.RegisterCommand('przywrocpostac', {'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin'}, function(xPlayer, args, showError)
...
ESX.RegisterCommand('revivedist', {'trialsupport', 'support', 'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin', 'best'}, function(xPlayer, args, showError)
    if args.dist then
		if args.dist <= 500 then
			local admincoords = GetEntityCoords(GetPlayerPed(xPlayer.source))
			exports['cookie_logs']:SendLog(xPlayer.source, "Użyto komendy /revivedist " .. tonumber(args.dist), "revive")
...
ESX.RegisterCommand('bitkiadd', {'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin'}, function(xPlayer, args, showError)
		...
		MySQL.query('SELECT wins,loses FROM bitki WHERE org_name = @org_name', {
			...
			if result[1] ~= nil then
				if args.what == 'win' then
					...
					exports['cookie_logs']:SendLog(source, xPlayer.name..' dodal '..args.wartosc..' winy dla organizacji '..args.job, 'bitkimanage', '5793266')
				elseif args.what == 'lose' then
					...
					exports['cookie_logs']:SendLog(source, xPlayer.name..' dodal '..args.wartosc..' lose dla organizacji '..args.job, 'bitkimanage', '5793266')
				end
			end
		end)
	end
...
ESX.RegisterCommand('bitkiremove', {'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin', 'best'}, function(xPlayer, args, showError)
	if args.job ~= nil and args.wartosc ~= nil and args.wartosc ~= nil then
		MySQL.query('SELECT wins,loses FROM bitki WHERE org_name = @org_name', {
			['@org_name'] = args.job
		}, function(result)
			if result[1] ~= nil then
				if args.what == 'win' then
					...
					exports['cookie_logs']:SendLog(source, xPlayer.name..' usunal '..args.wartosc..' winow dla organizacji '..args.job, 'bitkimanage', '5793266')
				elseif args.what == 'lose' then
					MySQL.update('UPDATE bitki SET loses = @loses WHERE org_name = @org_name',{ 
						['@loses'] = result[1].loses - args.wartosc,
						['@org_name'] = args.job
					})
					exports['cookie_logs']:SendLog(source, xPlayer.name..' usunal '..args.wartosc..' lose dla organizacji '..args.job, 'bitkimanage', '5793266')
				end
			end
		end)
	end
...
ESX.RegisterCommand('updateorgname', {'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin', 'best'}, function(xPlayer, args, showError)
...
ESX.RegisterCommand('spawn', {'trialsupport', 'support', 'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin'}, function(xPlayer, args, showError)
