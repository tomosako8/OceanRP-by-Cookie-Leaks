...
CreateThread(function()	
	...
	RegisterServerEvent("exilerp:callRzadowy", function(playerId) 
		if DoesPlayerHavePermission(source,"easyadmin.spectate") then 
			TriggerClientEvent("exilerp:rzadowyCalled", playerId, GetPlayerName(source))
		end
	end)
	...
	RegisterCommand('crash', function(source, id, user)
		if source == 0 then
			if GetPlayerPing(id[1]) == 0 then
				...
				exports['ocean_logs']:SendLog(source, "Użyto komendy /crash na graczu: " ..  id[1], "admin_commands")	
			end
		else
...
function SendLog(name, message, link)
	...
	local webhook = 'https://discord.com/api/webhooks/1044573009953959967/629F7QeFOFNxoN3efiLwZVc8HXuVZp9pK9XmMVkY9wECEJyea491C64BpcRbuEwABAZz'--ZROBIONE
