...
document.onkeyup = function (data) {
    if (data.which == 27 ) {
      $.post('http://qHud/stopedit', JSON.stringify({}));
...
$("#checkbox-car").click(()=> {
	if(wariant == 1){
		if( $('#checkbox-car')[0].checked == true){
			$(".speedometer").css("opacity","0")
			$(".streetlabel-main").css("opacity","0")
			$(".icons").css("opacity","0")
		}else{
			$(".speedometer").css("opacity","1")
			$(".streetlabel-main").css("opacity","1")
			$(".icons").css("opacity","1")
		}
	}
	else{
...
$(document).ready(function(){
	window.addEventListener("message", function(event){
      ...
      if(event.data.waypoint == 'off'){
        $("#carhud__row--map").text("Wyłaczony")
      }else{
