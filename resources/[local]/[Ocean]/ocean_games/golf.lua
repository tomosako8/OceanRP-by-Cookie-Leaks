...
CreateThread(function()
	...
	while true do
		...
		local distance, sleep = #(playerLoc - vec3(-1332.7823486328,128.18229675293,56.032329559326)), true
		if distance < 10.0 then
			sleep = false
			DrawMarker(27,-1332.7823486328,128.18229675293,56.052329559326, 0, 0, 0, 0, 0, 0, 1.5, 1.5, 10.3, 0, 519, 0, 105, 0, 0, 2, 0, 0, 0, 0)
		end
		...
		if distance < 1.5 then
			...
			if golfplaying then
				DisplayHelpText("Naciśnij ~g~~INPUT_CONTEXT~~s~ aby zakończyć grę w golfa.")
			else
				DisplayHelpText("Naciśnij ~g~~INPUT_CONTEXT~~s~ aby zacząć grę w golfa.")
			end
			if (IsControlJustReleased(1, 38)) then
				...
				if (golfplaying) then
					...
					TriggerEvent("customNotification","Przytrzymaj E aby uderzyć, A-D aby obrócić, Y aby zmienić kij.")
				end
			end
...
function spawnCart()
	RequestModel('caddy')
	while not HasModelLoaded('caddy') do
		Citizen.Wait(0)
	end
	local spawncordy = vector3(-1332.7823486328, 128.18229675293, 56.032329559326)
	ESX.Game.SpawnVehicle('caddy', spawncordy, 180, function(spawned_car)
		--local spawned_car = Citizen.InvokeNative(0xAF35D0D2583051B0, 'caddy', -1332.7823486328,128.18229675293,56.032329559326, 180, true, false)
...
function createBall(x,y,z)
	...
	mygolfball = Citizen.InvokeNative(0x509D5878EB39E842, `prop_golf_ball`, x, y, z, true, true, false)
	...
	while golfstate == 1 do
		...
		if GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
			...
			if distance < 50.0 then
				DisplayHelpText("Przejdź do swojej piłki, naciśnij ~g~E~s~ aby położyć nową piłkę jeśli utknąłeś.")
				if ( IsControlJustReleased(1, 38) ) then
					dropShot()
				end
...
function endShot()
	...
	if distance < 1.5 then
		TriggerEvent("customNotification","Piłka wpadła do dołka albo była w zasięgu dołka!")
		totalgolfstrokes = golfstrokes + totalgolfstrokes
		golfstrokes = 0
		ballstate = 1
	end
	if golfstrokes > 12 then
		TriggerEvent("customNotification","Zrobiłeś zbyt wiele uderzeń")
		totalgolfstrokes = golfstrokes + totalgolfstrokes
		golfstrokes = 0
		ballstate = 1
	end
...
function dropShot()
	...
	while doingdrop do
		...
		if distance < 50.0 and distanceHole > 50.0 then
			DisplayHelpText("Naciśnij ~g~E~s~ aby upuścić tu piłkę.")
			if ( IsControlJustReleased(1, 38) ) then
				doingdrop = false
			...
			DisplayHelpText("Naciśnij ~g~E~s~ aby upuścić nową piłkę - ~r~ jesteś za daleko od piłki lub za blisko dołka.")
		end
	end
end
...
function addBallBlip()
	...
	AddTextComponentString(tostring("Piłka"))
...
function BlipsStartEnd()
	...
	BeginTextCommandSetBlipName("STRING");
	AddTextComponentString(tostring("Początek dołka"))
	...
	BeginTextCommandSetBlipName("STRING");
	AddTextComponentString(tostring("Koniec dołka"))
...
function addblipGC()
	...
	BeginTextCommandSetBlipName("STRING");
	AddTextComponentString(tostring("Pole golfowe"))
...
AddEventHandler('customNotification', function(response)
	TriggerEvent("chatMessage", '^*^0[^3Centrala^0] ', { 44, 7, 56 }, "GOLF: "..response)
end)
...
AddEventHandler('attachProp', function(attachModelSent,boneNumberSent,x,y,z,xR,yR,zR)
	...
	attachedProp = Citizen.InvokeNative(0x509D5878EB39E842, attachModel, 1.0, 1.0, 1.0, 1, 1, 0)
