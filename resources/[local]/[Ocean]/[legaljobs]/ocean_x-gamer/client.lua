...
local Cfg = {
	CollectSeeds = {
		{
			coords = vector3(2848.24, 4578.14, 46.35),
		},
	},
	TransferingSeeds = {
		{
			coords = vector3(-630.29, 224.85, 81.88-0.95),
		},
	},
	Garage = {
		{
			coords = vector3(2826.57, 4569.31, 45.60)
		},
		{
			coords = vector3(-632.38, 252.19, 80.65)
		},
	},
	BossActions = {
		{
	...
	Cloakroom = {
		{
			coords = vector3(-634.59, 226.89, 81.88-0.95)
		},
		{
			coords = vector3(2833.06, 4571.24, 45.60)
		},
	},
	Clothes = {
		Male = {
...
local Blips = {
	{title="#1 Zbieranie Ziaren Kawy", colour=2, id=143, see = true, coords = vector3(2848.24, 4578.14, 46.35)},
	{title="#2 Przerabianie Ziaren Kawy", colour=2, id=143, see = true, coords = vector3(-630.29, 224.85, 81.88-0.95)}
}
...
CreateThread(function()
	...
	while true do
		if PlayerData.secondjob.name == 'x-gamer' then
...
refreshBlips = function()
	if PlayerData.secondjob ~= nil and PlayerData.secondjob.name == 'x-gamer' then
		for i=1, #Blips, 1 do
			if Blips[i].see then
	...
	while true do
		...
		if PlayerData.secondjob ~= nil and PlayerData.secondjob.name == 'x-gamer' then
			...
			if isInMarker and inAction then
				TriggerEvent('exile_legalxgamer:hasEnteredMarker', 'exitMarker')
			end
...
CreateThread(function()
	while true do
		Citizen.Wait(10)
		if PlayerData.secondjob and PlayerData.secondjob.name == 'x-gamer' then
...
StartCollect = function()
	local playerPed = PlayerPedId()
	TriggerServerEvent('exile_legalxgamer:collect', 'ziarnokawy')
...
TransferingSeeds = function()
	...
	for i=1, #inventory, 1 do
		if inventory[i].name == 'ziarnokawy' then
	...
	if count >= 100 then
		...
		ESX.ShowNotification('~r~Potrzebujesz 100 Ziaren Kawy aby móć zacząć pażyć kawę!')
	end
end
...
CarOut = function()
	...
	if IsPedInAnyVehicle(player, false) then
		...
		if IsVehicleModel(carCafe, GetHashKey("gmcat4")) then
		...
		if ESX.Game.IsSpawnPointClear(GetEntityCoords(PlayerPedId()), 7) then
			ESX.Game.SpawnVehicle('gmcat4', GetEntityCoords(PlayerPedId()), 227.04, function(vehicle)
...
OpenBossMenu = function()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'x-gamer_boss', {
		...
		if data.current.value == '1' then
			if PlayerData.secondjob.grade == 7 then
				TriggerEvent('esx_society:openSecondBossMenu', 'x-gamer', function(data, menu)
					menu.close()
			...
			ESX.TriggerServerCallback('ExileRP:getCourses', function(kursy)
				if kursy then
					...
					for i=1, #kursy, 1 do
						...
						ESX.UI.Menu.Open('list', GetCurrentResourceName(), 'x-gamer_courses', elements, function(data, menu)
						end, function(data, menu)
							menu.close()
						end)
                    end
...
AddEventHandler('exile_legalxgamer:hasEnteredMarker', function(zone, number)
	...
	if zone == 'CollectSeeds' then
		...
		CurrentActionMsg	= "Naciśnij ~INPUT_CONTEXT~, aby zebrać ~g~Ziarna Kawy"
		CurrentActionData	= {}
	end
	...
	if zone == 'TransferingSeeds' then
		...
		CurrentActionMsg	= "Naciśnij ~INPUT_CONTEXT~, aby przerobić ~g~Ziarna Kawy"
		CurrentActionData	= {}
	end
...
OpenMobilexgamerActionsMenu = function()
	...
	if IsVehicleModel(vehicle, GetHashKey("gmcat4")) then
