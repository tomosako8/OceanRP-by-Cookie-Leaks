...
AddEventHandler('exile_weazel:hasEnteredMarker', function(zone, data)
	...
	elseif zone == 'secondjobZone' then
		CurrentAction     = 'secondjob_zone'
...
CreateThread(function()
	while true do
        ...
        if PlayerData.secondjob ~= nil and PlayerData.secondjob.name == 'weazel' then
			...
			if secondjobStarted ~= nil then
				if secondjobStarted.start_point ~= nil then
					if #(GetEntityCoords(PlayerPedId()) - secondjobStarted.start_point) < 15.0 then
						found = true
						ESX.DrawBigMarker(secondjobStarted.start_point)
					end
				end
			end
			...
			if secondjobStarted ~= nil then
				if secondjobStarted.start_point ~= nil then
					if #(coords - secondjobStarted.start_point) < 3.0 then
						isInMarker  = true
						currentZone = 'secondjobZone'
					end
				end
			end
...
RegisterCommand('-weazelf6', function(source, args, rawCommand)
	if PlayerData.secondjob and PlayerData.secondjob.name == 'weazel' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'weazel_actions') then
		WeazelActionsMenu()
	end
end, false)

RegisterKeyMapping('-weazelf6', 'Włącz/wyłącz menu (Weazel News)', 'keyboard', 'F6')
			...
			if CurrentAction ~= nil then
				...
				if IsControlJustReleased(0, Keys['E']) then
					...
					elseif CurrentAction == 'cloakroom' then
						OpenCloakroom()
					elseif CurrentAction == 'secondjob_zone' then
...
StartPhoto = function()
	...
	exports['OceanRP']:DrawProcent(1200, function()
...
WeazelActionsMenu = function()
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'weazel_actions', {
		...
		elseif data.current.value == 'start_secondjob' then
			if secondjobStarted ~= nil then
				Cancelsecondjob()
			else
				if PlayerData.secondjob ~= nil and PlayerData.secondjob.name == 'weazel' then
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'weazel_boss', {
		...
		if data.current.value == '1' then
			if PlayerData.secondjob.grade >= 7 then
				TriggerEvent('esx_society:openSecondBossMenu', 'weazel', function(data, menu)
					menu.close()
        ...
        elseif data.current.value == '2' then
			menu.close()
			ESX.TriggerServerCallback('OceanRP:getCourses', function(kursy)
				if kursy then
					local elements = {
...
CreateNewPlace = function(state)
	if state == 'START' then
		...
		secondjobStarted.start_point = Config.MissionLocations[math.random(1, #Config.MissionLocations)]

		--ETAP 1
		blip_work = AddBlipForCoord(vec3(secondjobStarted.start_point.x, secondjobStarted.start_point.y, secondjobStarted.start_point.z))
		...
		local street = GetStreetNameFromHashKey(GetStreetNameAtCoord(secondjobStarted.start_point.x, secondjobStarted.start_point.y, secondjobStarted.start_point.z))
