...
local Limits = {20, 40, 60, 80, 100, 140, 180, 220, 260, 300}
...
AddEventHandler(GetCurrentResourceName() .. ':exitedMarker', function(zone)
	if isUsing then
		...
		elseif Config.Zones[PlayerData.secondjob.name] then
			TriggerServerEvent('exile:setUsed', zoneName, 'society_'..PlayerData.secondjob.name, false)
		end
...
CreateThread(function()
	while true do
		...
		if PlayerData.job ~= nil then
			if Config.Zones[PlayerData.job.name] or Config.Zones[PlayerData.secondjob.name] then
				local coords, letSleep = GetEntityCoords(PlayerPedId()), true
				local currentJob = ""
				if Config.Zones[PlayerData.job.name] then
					currentJob = PlayerData.job.name
				elseif Config.Zones[PlayerData.secondjob.name] then
					currentJob = PlayerData.secondjob.name
				end
	...
	while true do
		...
		if PlayerData.job ~= nil then
			if Config.Zones[PlayerData.job.name] or Config.Zones[PlayerData.secondjob.name] then
				...
				elseif Config.Zones[PlayerData.secondjob.name] then
					currentJob = PlayerData.secondjob.name
					currentLabel = PlayerData.secondjob.label
				end
				for k,v in pairs(Config.Zones[currentJob]) do
					if k ~= 'secondjob' then
						if k == 'DeleteVehicles' or k == 'DeleteBoats' then
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and PlayerData.job then
				if (Config.Zones[PlayerData.job.name] or Config.Zones[PlayerData.secondjob.name]) then
					...
					elseif Config.Zones[PlayerData.secondjob.name] then
						currentJob = PlayerData.secondjob.name
						currentGrade = PlayerData.secondjob.grade
					end
...
CreateThread(function()
	while true do
		Wait(0)
		if not IsPedInAnyVehicle(PlayerPedId()) and ESX.GetPlayerData().job and (Config.Interactions[ESX.GetPlayerData().job.name] or Config.Interactions[ESX.GetPlayerData().secondjob.name]) then
			if not IsPedInAnyVehicle(PlayerPedId()) then
				local Control = 0
				if Config.Interactions[ESX.GetPlayerData().job.name] then
					Control = 167
				elseif Config.Interactions[ESX.GetPlayerData().secondjob.name] then
					Control = 168
				end
				if IsControlJustReleased(0, Control) and GetEntityHealth(PlayerPedId()) > 100 then
					if Config.Interactions[ESX.GetPlayerData().job.name] then
						OpenActionsMenu(ESX.GetPlayerData().job.name, ESX.GetPlayerData().job.grade)
					elseif Config.Interactions[ESX.GetPlayerData().secondjob.name] then
						OpenActionsMenu(ESX.GetPlayerData().secondjob.name, ESX.GetPlayerData().secondjob.grade)
					end
				end
			else
				Wait(200)
		...
		if Config.Zones[job].secondjob == true then
			if job == 'psycholog' then
				if grade >= Config.Zones[job].BossMenu.from then 
					TriggerEvent('esx_society:openSecondBossMenu', job, function(data, menu)
						menu.close()
			...
			elseif PlayerData.secondjob.name ~= nil then
				TriggerEvent('esx_society:openBossMenu', job, function(data, menu)
					menu.close()
				end, { showmoney = false, withdraw = false, deposit = true, wash = false, employees = false, grades = false })
			end
...
Config.Coords = vector3(-552.91, -191.45, 37.25)
