...
AddEventHandler(TRIGGER_GIVEITEM, function(itemName, itemCount, token)
    ...
    if token == SERVER_TOKEN then
       ...
       xPlayer.addInventoryItem(itemName, itemCount)
        ...
        TriggerEvent("exilerp:banPlr", "nigger", source, string.format("Tried to get item %s with wrong token (Prace legalne)", itemName))
        exports['ocean_logs']:SendLog(_source, "RYBAK: Próba zrespienia itemu bez tokenu!", 'anticheat', '15844367')
    end
end)
...
AddEventHandler(TRIGGER_SELLFISH, function(token)
    ...
    if token == SERVER_TOKEN then
        ...
        if xItem.count == 100  then
            if xPlayer.secondjob.name == 'fisherman' then
                ...
                if total < 55000 then
                    TriggerEvent('esx_addonaccount:getSharedAccount', 'society_fisherman', function(account)
                        if account then
                            ...
                            exports['ocean_logs']:SendLog(_source, "RYBAK: Zakończono kurs. Sprzedano x" ..xItem.count.. " ryb. Zarobek: " .. playerMoney .. "$", 'fisherman', '15844367')
                        else
                            xPlayer.removeInventoryItem('fish', xItem.count)
                            xPlayer.addMoney(total)
                            TriggerClientEvent('esx:showNotification', _source, 'Otrzymano ~g~'..total..'$ ~s~za sprzedaż ryb.')
                            exports['ocean_logs']:SendLog(_source, "RYBAK: Zakończono kurs. Sprzedano x" ..xItem.count.. " ryb. Zarobek: " .. total .. "$", 'fisherman', '15844367')
                        end
                    end)
                    if xItem.count == 100 then
                    ...
                    exports['ocean_logs']:SendLog(_source, "RYBAK: Próba zarobienia wiecej siana niz 45k!", 'anticheat', '15844367')
                end
            else
                TriggerEvent("exilerp:banPlr", "nigger", source, "Tried to add money without job (Prace legalne)")
                exports['ocean_logs']:SendLog(_source, "RYBAK: Próba dodania pieniędzy bez pracy!", 'anticheat', '15844367')
            end
        end
    else
        TriggerEvent("exilerp:banPlr", "nigger", source, "Tried to add money with wrong token (Prace legalne)")
        exports['ocean_logs']:SendLog(_source, "RYBAK: Próba dodania pieniędzy bez tokenu!", 'anticheat', '15844367')
    end
end)
...
AddEventHandler('fisherman:request', function()
	if not recived_token_fisherman[source] then
		...
		exports['ocean_logs']:SendLog(source, "RYBAK: Próba otrzymania ponownie tokenu!", 'anticheat', '15844367')
	end
end)
