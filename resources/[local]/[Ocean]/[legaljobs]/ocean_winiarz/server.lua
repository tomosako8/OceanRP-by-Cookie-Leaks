...
local TRIGGER_SELL = "exile_legalwiniarz:SellCoffee"..math.random(9999,999999)
local TRIGGER_TRANSFERING = "exile_legalwiniarz:Transfering"..math.random(9999,999999)
	...
	SetTimeout(60000, function()
		if PlayersHarvesting[source] == true then
			...
			if item.limit ~= -1 and item.count >= item.limit then
                ...
                TriggerClientEvent('exile_legalwiniarz:Cancel', source)
			else
...
RegisterServerEvent('exile_legalwiniarz:collect')
AddEventHandler('exile_legalwiniarz:collect', function(name)
	local _source = source
    if name == "pear" then
        local xPlayer  = ESX.GetPlayerFromId(source)
        if xPlayer.getInventoryItem('pear').count == 0 then
            PlayersHarvesting[_source] = true
            xPlayer.showNotification('~y~Trwa zbieranie winogron')
            Harvest(_source, name)
        else
            xPlayer.showNotification('~r~Nie możesz zbierać winogron posiadając winogrono!')
        end
    else
...
AddEventHandler(TRIGGER_TRANSFERING, function(countBefore, token)
    ...
    if SERVER_TOKEN == token then
        if xPlayer.secondjob.name == 'winiarz' then
            if xPlayer.getInventoryItem('winogrono2').count == 0 then
                if countBefore >= 100 then
                    local itemCount = math.floor((countBefore / 10))
                    xPlayer.removeInventoryItem('pear', itemCount * 10)
                    Paycheck('winiarz', xPlayer)
                    if itemCount >= 1 then
                        TriggerEvent('ExileRP:saveCours', xPlayer.secondjob.name, xPlayer.secondjob.grade, xPlayer.source)
                    end
                else
                    exports['ocean_logs']:SendLog(_source, "WINIARZ OSTRZEŻENIE: Próba zbugowania przeróbki!", 'anticheat', '15844367')
                end
            else
                xPlayer.showNotification('~r~Nie możesz przerabiać winogron posiadając winogrono!')
            end
        else
...
Paycheck = function(secondjob, xPlayer)
	...
	if secondjob == 'winiarz' then
        ...
        exports['ocean_logs']:SendLog(xPlayer.source, "WINIARZ: Zakończono kurs. Zarobek: " ..total.. "$", 'winiarz', '15844367')
	end
end

RegisterServerEvent('exile_legalwiniarz:stopPickup')
AddEventHandler('exile_legalwiniarz:stopPickup', function(zone)
	local _source = source
	PlayersHarvesting[_source] = nil
    TriggerClientEvent('exile_legalwiniarz:Cancel', source)
end)

RegisterServerEvent('exile_legalwiniarz:request')
AddEventHandler('exile_legalwiniarz:request', function()
	if not recived_token_winiarz[source] then
		TriggerClientEvent("exile_legalwiniarz:getrequest", source, SERVER_TOKEN, TRIGGER_SELL, TRIGGER_TRANSFERING)
...
RegisterServerEvent('exile_legalwiniarz:insertPlayer')
AddEventHandler('exile_legalwiniarz:insertPlayer', function(tablice)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
ESX.RegisterServerCallback('exile_legalwiniarz:checkSiedzacy', function(source, cb)
	cb(SmiecieSiedzace)
end)