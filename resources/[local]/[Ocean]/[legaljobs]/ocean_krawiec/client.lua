...
local Cfg = {
	...
	TransferingSeeds = {
		{
			coords = vector3(713.25, -971.06, 29.4628),
		},
	},
	Garage = {
		{
...
local Blips = {
	{title="#1 Szatnia", colour=2, id=143, see = true, coords = vector3(708.7081, -959.5554, 29.4453)},
	{title="#2 Zbieranie materiału", colour=2, id=143, see = true, coords = vector3(185.72, -1477.31, 29.14)},
	{title="#3 Przeróbka materiału", colour=2, id=143, see = true, coords = vector3(740.9933, -969.8665, 23.5263)}
}
...
CreateThread(function()
	...
	while true do
		if PlayerData.secondjob.name == 'krawiec' then
...
refreshBlips = function()
	if PlayerData.secondjob ~= nil and PlayerData.secondjob.name == 'krawiec' then
		for i=1, #Blips, 1 do
			if Blips[i].see then
	...
	while true do
		...
		if PlayerData.secondjob ~= nil and PlayerData.secondjob.name == 'krawiec' then
			...
			if isInMarker and inAction then
				TriggerEvent('exile_legalkrawiec:hasEnteredMarker', 'exitMarker')
			end
...
CreateThread(function()
	while true do
		Citizen.Wait(10)
		if PlayerData.secondjob and PlayerData.secondjob.name == 'krawiec' then
...
CarOut = function()
	...
	if IsPedInAnyVehicle(player, false) then
		...
		if IsVehicleModel(carCafe, GetHashKey("flybeliodas")) then
		...
		if ESX.Game.IsSpawnPointClear(GetEntityCoords(PlayerPedId()), 7) then
			ESX.Game.SpawnVehicle('flybeliodas', GetEntityCoords(PlayerPedId()), 227.04, function(vehicle)
...
OpenBossMenu = function()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'krawiec_boss', {
		...
		if data.current.value == '1' then
			if PlayerData.secondjob.grade == 7 then
				TriggerEvent('esx_society:openSecondBossMenu', 'krawiec', function(data, menu)
					menu.close()
...
OpenMobileKrawiecActionsMenu = function()
	...
	if IsVehicleModel(vehicle, GetHashKey("flybeliodas")) then
