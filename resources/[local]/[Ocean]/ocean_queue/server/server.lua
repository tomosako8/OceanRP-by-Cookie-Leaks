...
local UbWebhook = 'https://discord.com/api/webhooks/1028282230985986050/e_0q2sOwvCfQa34hgzQHmkvC8J3tvz3_s0R0qWaaCn5nmNt-WwOsg09srDpY0iuDc8YX'
local BanWebhook = 'https://discord.com/api/webhooks/1028282230985986050/e_0q2sOwvCfQa34hgzQHmkvC8J3tvz3_s0R0qWaaCn5nmNt-WwOsg09srDpY0iuDc8YX'
local BiletWebhook = 'https://discord.com/api/webhooks/1028282230985986050/e_0q2sOwvCfQa34hgzQHmkvC8J3tvz3_s0R0qWaaCn5nmNt-WwOsg09srDpY0iuDc8YX'
...
AddEventHandler('banCheater', function(playerId,reason)
	...
	exports['ocean_logs']:SendLog(playerId, "Został zbanowany permamentnie za "..reason, 'anticheat', '5793266')
end)
...
ESX.RegisterCommand('unban', {'mod', 'starszyadmin', 'starszymod', 'superadmin', 'admin'}, function(xPlayer, args, showError)
	...
	if args.steamid then
		MySQL.update('UPDATE exile_bans SET isBanned=0 WHERE license=@license',
		{
			...
			if xPlayer then
				xPlayer.showNotification('[OceanRP-QUEUE] ~o~Gracz został odbanowany!')
				...
				PerformHttpRequest(UbWebhook, function(err, text, headers) end, 'POST', json.encode({username = "OceanRP", embeds = UbEmbded}), { ['Content-Type'] = 'application/json' })
			else
				local prompt = 'PROMPT'
				local date = os.date('*t')				
				if date.month < 10 then date.month = '0' .. tostring(date.month) end
				...
				PerformHttpRequest(UbWebhook, function(err, text, headers) end, 'POST', json.encode({username = "OceanRP", embeds = UbEmbded2}), { ['Content-Type'] = 'application/json' })
			end
		end)
	end
...
ESX.RegisterCommand('banhex', {'mod', 'starszyadmin', 'starszymod', 'superadmin', 'admin', 'best', 'support'}, function(xPlayer, args, showError)
	local xPlayer = xPlayer
	if args.steamid and tonumber(args.czas) and args.powod then
		...
		if string.find(steamID, 'steam:') then
			steamFound = true
			if xPlayer then
				xPlayer.showNotification('[OceanRP-QUEUE] ~o~Nie możesz zbanować użytkownika po steam ID. Użyj jego licencji!')
			end
		end
		if tPlayer and steamFound == false then
			...
			if xPlayer == false then
				bannedby = "Administracja OceanRP"
			else
			...
			MySQL.update('UPDATE exile_bans SET reason=@reason, expired=@expired, bannedby=@bannedby, isBanned=1 WHERE license=@license', {
				...
				if xPlayer then
					xPlayer.showNotification('[OceanRP-QUEUE] ~o~Gracz został zbanowany')
				end
				local date = os.date('*t')			
				if date.month < 10 then date.month = '0' .. tostring(date.month) end
				if date.day < 10 then date.day = '0' .. tostring(date.day) end
				...
				PerformHttpRequest(BanWebhook, function(err, text, headers) end, 'POST', json.encode({username = "OceanRP", embeds = BanEmbded}), { ['Content-Type'] = 'application/json' })
			end)
		...
		elseif not tPlayer and steamFound == false then
			...
			MySQL.update('UPDATE exile_bans SET reason=@reason, expired=@expired, bannedby=@bannedby, isBanned=1 WHERE license=@license', {
				...
				PerformHttpRequest(BanWebhook, function(err, text, headers) end, 'POST', json.encode({username = "OceanRP", embeds = BanEmbded1}), { ['Content-Type'] = 'application/json' })
			end)
		end
	end
	...
	if args.licenseid and args.steamid and args.discordid and tonumber(args.czas) and args.powod then
		...
		if not tPlayer then
			...
			MySQL.update('UPDATE exile_bans SET reason=@reason, expired=@expired, bannedby=@bannedby, isBanned=1 WHERE license=@license', {
				...
				if xPlayer then
					xPlayer.showNotification('[OceanRP-QUEUE] ~o~Cheater został zbanowany')
				end
				local date = os.date('*t')			
				if date.month < 10 then date.month = '0' .. tostring(date.month) end
				if date.day < 10 then date.day = '0' .. tostring(date.day) end
...
ESX.RegisterCommand('banid', {'mod', 'starszyadmin', 'starszymod', 'superadmin', 'admin' , 'best', 'support'}, function(xPlayer, args, showError)
	local xPlayer = xPlayer
	if tonumber(args.id) and tonumber(args.czas) and args.powod then
		...
		if tPlayer then
			...
			MySQL.update('UPDATE exile_bans SET reason=@reason, expired=@expired, bannedby=@bannedby, isBanned=1 WHERE license=@license', {
				...
				PerformHttpRequest(BanWebhook, function(err, text, headers) end, 'POST', json.encode({username = "OceanRP", embeds = BanEmbded2}), { ['Content-Type'] = 'application/json' })
			end)
	...
	if args.steamid ~= nil then
		...
		MySQL.Async.fetchAll('SELECT * FROM whitelist WHERE identifier = @identifier', {
			...
			if result[1] ~= nil then
				if xPlayer then
					xPlayer.showNotification('~r~[OceanRP-QUEUE] ~o~The player is already whitelisted on this server!')
				end
			else
				MySQL.Async.execute('INSERT INTO whitelist (identifier, ticket, back, discord) VALUES (@identifier, @ticket, @back, @discord)', {
					...
					if xPlayer then
						xPlayer.showNotification('~r~[OceanRP-QUEUE] ~o~Dodano gracza: '..steamID..' na whiteliste!')
						local administrator = GetPlayerName(xPlayer.source)
						local date = os.date('*t')		
						if date.month < 10 then date.month = '0' .. tostring(date.month) end
						if date.day < 10 then date.day = '0' .. tostring(date.day) end
						...
						local WLaddEmbded = {
							{
								...
								["footer"] = {
								["text"] = "OceanRP WL System - " ..date.."",
								},
							}
						}
							
						PerformHttpRequest(BiletWebhook, function(err, text, headers) end, 'POST', json.encode({username = "OceanRP", embeds = WLaddEmbded}), { ['Content-Type'] = 'application/json' })
					else
						local prompt = 'PROMPT'
						local date = os.date('*t')		
						if date.month < 10 then date.month = '0' .. tostring(date.month) end
						...
						PerformHttpRequest(BiletWebhook, function(err, text, headers) end, 'POST', json.encode({username = "OceanRP", embeds = WLaddEmbded1}), { ['Content-Type'] = 'application/json' })
					end
				end)
			end
		end)
...
ESX.RegisterCommand('bilet', {'superadmin', 'best'}, function(xPlayer, args, showError)
	if args.steamid and args.bilety then
		MySQL.update('UPDATE whitelist SET ticket = @ticket WHERE identifier = @identifier', {
			...
			if xPlayer then
				xPlayer.showNotification('~r~[OceanRP-QUEUE] ~o~Gracz: '..args.steamid..' dostał '..args.bilety..' ticketów!')
				local administrator = GetPlayerName(xPlayer.source)
				local date = os.date('*t')		
				if date.month < 10 then date.month = '0' .. tostring(date.month) end
				if date.day < 10 then date.day = '0' .. tostring(date.day) end
				...
				PerformHttpRequest(BiletWebhook, function(err, text, headers) end, 'POST', json.encode({username = "OceanRP", embeds = BiletEmbded}), { ['Content-Type'] = 'application/json' })
			else
				local prompt = 'PROMPT'
				local date = os.date('*t')		
				if date.month < 10 then date.month = '0' .. tostring(date.month) end
				...
				PerformHttpRequest(BiletWebhook, function(err, text, headers) end, 'POST', json.encode({username = "OceanRP", embeds = BiletEmbded1}), { ['Content-Type'] = 'application/json' })
			end
		end)
	end
...
ESX.RegisterCommand('powrotny', {'superadmin', 'best'}, function(xPlayer, args, showError)
	if args.steamid and args.wartosc then
		MySQL.update('UPDATE whitelist SET back = @back WHERE identifier = @identifier', {
			...
			if xPlayer then
				xPlayer.showNotification('~r~[OceanRP-QUEUE] ~o~Gracz: '..args.steamid..' dostał '..args.wartosc..' ticketów!')
				local administrator = GetPlayerName(xPlayer.source)
				local date = os.date('*t')		
				if date.month < 10 then date.month = '0' .. tostring(date.month) end
				if date.day < 10 then date.day = '0' .. tostring(date.day) end
