...
AddEventHandler('esx_sellnpc:sell', function()
	...
	local sheriff = exports['esx_scoreboard']:CounterPlayers('sheriff')
	...
	if police+sheriff >= Config.LSPD then
		TriggerClientEvent('esx_sellnpc:sell', _source, zainteresowany, psy, police + sheriff, fotka)
	else
...
AddEventHandler('esx_sellnpc:acceptSell', function(cops)
	...
	if interested[_source] ~= nil then
		...
		if drugType == 'meth_pooch' then
			blackMoney = math.random(1300, 1600) * x
		elseif drugType == 'coke_pooch' then
			blackMoney = math.random(1666, 1800) * x
		elseif drugType == 'cokeperico_pooch' then
			blackMoney = math.random(1666, 1800) * x
		elseif drugType == 'opium_pooch' then
			blackMoney = math.random(3900, 4500) * x
		elseif drugType == 'weed_pooch' then
			blackMoney = math.random(800, 900) * x
		elseif drugType == 'oghaze_pooch' then
			blackMoney = math.random(900, 1200) * x
		elseif drugType == 'exctasy_pooch' then
			blackMoney = math.random(1200, 1300) * x
		end
