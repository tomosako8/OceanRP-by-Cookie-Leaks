...
local Thread     = nil
local czas = true
local LastDeal = GetGameTimer() - 5 * 15000
...
CreateThread(function()
	while true do
		...
		if CanSell and TargetPed then
			if GetEntityModel(TargetPed) == GetHashKey('hc_hacker') or GetEntityModel(TargetPed) == GetHashKey('s_m_m_doctor_01') or GetEntityModel(TargetPed) == GetHashKey('s_m_y_casino_01') or GetEntityModel(TargetPed) == GetHashKey('s_m_y_pilot_01') or GetEntityModel(TargetPed) == GetHashKey('s_m_y_barman_01') or GetEntityModel(TargetPed) == GetHashKey('s_m_o_busker_01') or  GetEntityModel(TargetPed) == GetHashKey('a_m_y_business_02') or GetEntityModel(TargetPed) == GetHashKey('s_m_y_barman_01') or GetEntityModel(TargetPed) == GetHashKey('mp_m_shopkeep_01') or GetEntityModel(TargetPed) == GetHashKey('a_f_y_business_01') or GetEntityModel(TargetPed) == GetHashKey('cs_solomon') or GetEntityModel(TargetPed) == GetHashKey('csb_tomcasino') or GetEntityModel(TargetPed) == GetHashKey('s_m_m_strpreach_01') or GetEntityModel(TargetPed) == GetHashKey('s_m_m_paramedic_01') or GetEntityModel(TargetPed) == GetHashKey('ig_casey') or GetEntityModel(TargetPed) == GetHashKey('s_m_y_ammucity_01') then
				print('[oceanrp] tried to sell narcotic to blacklisted NPC')
			else
		...
		if CanSell and TargetPed then
			for k,v in pairs(CantPeds) do
				if GetEntityModel(TargetPed) == GetHashKey(v) then
					cansellnarko = false
					return
				end
			end
			if cansellnarko ~= false then
				print('moze')
				SetTextComponentFormat('STRING')
...
AddEventHandler('esx_sellnpc:sell', function(isInterested, notifyCops, cops, photo)
	...
	if not isInterested then		
		if isInterested ~= nil then
			...
			local sellingTime = GetGameTimer()+5000
			local canceled = false
			local callCopsAfter = GetGameTimer()+3500
			local copsCalled = false
			while sellingTime > GetGameTimer() do
				Citizen.Wait(3)
				...
				if GetGameTimer() > callCopsAfter and not copsCalled then
					copsCalled = true
					local ped = PlayerPedId()
	...
	elseif #(coords - GetEntityCoords(CurrentPed)) <= 2.5 then
		CreateThread(function()
			...
			ESX.ShowNotification('~y~Oferowanie narkotyków...')
			local ticks = GetGameTimer()+5000
			local canceled = false
			while ticks > GetGameTimer() do
				Citizen.Wait(3)
				ESX.ShowHelpNotification('Naciśnij ~INPUT_VEH_DUCK~ aby przerwać sprzedaż')
				DisableControlAction(2, 23, true)
				if IsControlJustPressed(0, 73) then
