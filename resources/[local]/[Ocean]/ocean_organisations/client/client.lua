...
CreateThread(function()
	...
	while ESX.GetPlayerData().thirdjob == nil do
		Citizen.Wait(100)
	end
...
CreateThread(function()
	while true do
		if PlayerData.thirdjob and string.find(PlayerData.thirdjob.name, "org") then
			jestwjakiesorg = true
		else
...
function refreshBlip()
	if PlayerData.thirdjob ~= nil and Config.Blips[PlayerData.thirdjob.name] then
		local blip = AddBlipForCoord(Config.Blips[PlayerData.thirdjob.name])
...
AddEventHandler('exile_organisations:hasEnteredMarker', function(zone)
	if zone == 'Cloakroom' then
		...
		CurrentActionMsg  = _U('cloakroom_menu', PlayerData.thirdjob.label)
		CurrentActionData = {}
	elseif zone =='Inventory' then
		CurrentAction = 'inventory'
		CurrentActionMsg  = _U('inventory_menu', PlayerData.thirdjob.label)
		CurrentActionData = {}
	elseif zone == 'Licenses' then
		CurrentAction = 'licenses'
		CurrentActionMsg  = _U('licenses_menu', PlayerData.thirdjob.label)
		CurrentActionData = {}
	elseif zone == 'BossMenu' then
		CurrentAction = 'boss_menu'
		CurrentActionMsg  = _U('boss_menu', PlayerData.thirdjob.label)
		CurrentActionData = {}
...
AddEventHandler('exile_organisations:hasExitedMarker', function(zone)
	if isUsing then
		isUsing = false
		TriggerServerEvent('exile:setUsed', zoneName, 'society_'..PlayerData.thirdjob.name, false)
	end
	zoneName = nil
	CurrentAction = nil
	...
	while true do
		...
		if PlayerData.thirdjob ~= nil then
			...
			if jestwjakiesorg then
				if Config.Zones[PlayerData.thirdjob.name] then
					for k,v in pairs(Config.Zones[PlayerData.thirdjob.name]) do
	...
	while true do
		...
		if jestwjakiesorg then
			if CurrentAction then
				...
				if IsControlJustPressed(0, 38) or IsDisabledControlJustPressed(0, 38) and PlayerData.thirdjob and Config.Zones[PlayerData.thirdjob.name] then

					if CurrentAction == 'cloakroom' then
						OpenCloakroomMenu('society_' .. PlayerData.thirdjob.name)
						print('1')
					elseif CurrentAction == 'MainMenu' then
						OpenJakJaKurwaNieNawidzeJebanegoFalsiakaMenu(PlayerData.thirdjob.name)
					...
					elseif CurrentAction == 'Blachy' then
						exports['OceanRP']:WybijBlachyMenu()
					end
...
RegisterCommand('showF7ORG', function()
	if PlayerData.thirdjob then
		if not IsPedInAnyVehicle(PlayerPedId()) and not exports['esx_policejob']:IsCuffed() and not exports['esx_ambulancejob']:isDead() then
			...
			end, 'society_' .. PlayerData.thirdjob.name)
		end
	end
end)
...
function OpenActionsMenu()
	...
	if PlayerData.thirdjob.name == 'org27' then
		elements = {
		{label = "Kajdanki", value = 'handcuffs'},
		{label = "Napraw pojazd", value = 'repair'},
		{label = "Użyj wytrychu", value = 'wytrych'},
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'actions_menu',
	{
		...
		elseif (data.current.value == 'worek') then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'actions_menu',
			{
				title    = PlayerData.thirdjob.label,
				align    = 'center',
				elements = {
...
withdrawDialog = function()
	...
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'deposit_black', {
		...
		TriggerServerEvent('exilerp:withdrawMoney', data.value)
		ESX.UI.Menu.CloseAll()
	end, function(data, menu)
		menu.close()
	end)
...
depositDialog = function()
	...
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'deposit_black', {
		...
		TriggerServerEvent('exilerp:depositMoney', data.value)
		ESX.UI.Menu.CloseAll()
	end, function(data, menu)
		menu.close()
	end)
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'zarzadzanie',
		{
			...
			elements = {
				...
				{label = "Wypłać Pieniądze z Konta Organizacji", value = 'withdraw'},
				{label = "Wpłać Pieniądze na Konto Organizacji", value = 'deposit'},
			}
		}, function(data, menu)
...
RegisterNetEvent('exilerp:showAccountMenu', function(par)
...
function OpenJakJaKurwaNieNawidzeJebanegoFalsiakaMenu(organization)
	local elements = {
		{label = "Zarządzaj Kontem Organizacji", value = 'manage'},
		{label = "Magazyn", value = 'storage'},
		{label = "Zarządzanie", value = 'managment'},
		{label = "Narkotyki", value = 'narkotyki'},
		{label = "Zestawy", value = 'kits'}
	}
	if (Config.Zones[PlayerData.thirdjob.name] and PlayerData.thirdjob.grade >= Config.Zones[PlayerData.thirdjob.name].Licenses.from) or Config.InstanceOrgs.Licenses.from then
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mejn',
	{
		...
		if data.current.value == 'storage' then
			OpenInventoryMenu('society_' .. PlayerData.thirdjob.name)
		elseif data.current.value == 'narkotyki' then
			OpenNarkoMenu()
		elseif(data.current.value == 'manage') then 
			TriggerServerEvent('exilerp:manageAccount')
		elseif data.current.value == 'managment' then
			OpenBossMenu(PlayerData.thirdjob.name, (Config.Zones[PlayerData.thirdjob.name] and Config.Zones[PlayerData.thirdjob.name].MainMenu.from) or Config.InstanceOrgs.MainMenu.from)
		elseif data.current.value == "licenses" then
			OpenLicensesMenu('society_' .. PlayerData.thirdjob.name)
		elseif data.current.value == 'depositall' then
			TriggerEvent('exile:putInventoryItems', 'society_' .. PlayerData.thirdjob.name)
		elseif data.current.value == 'kits' then
			if PlayerData.thirdjob.name ~= nil and PlayerData.thirdjob.grade > 0 then
				OpenKitsMenu(PlayerData.thirdjob.name)
			else
...
function OpenInventoryMenu(organization)
	...
	if (Config.Zones[PlayerData.thirdjob.name] and PlayerData.thirdjob.grade >= Config.Zones[PlayerData.thirdjob.name].Inventory.from) or PlayerData.thirdjob.grade >= Config.InstanceOrgs.Inventory.from then
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory',
	{
		...
		if isUsing then
			isUsing = false
			TriggerServerEvent('exile:setUsed', 'Inventories', 'society_'..PlayerData.thirdjob.name, false)
		end
	end)
end
...
function OpenWeaponsMenu(organization)
	...
	if (Config.Zones[PlayerData.thirdjob.name] and PlayerData.thirdjob.grade >= Config.Zones[PlayerData.thirdjob.name].Weapons.from) or Config.InstanceOrgs.Inventory.from then
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory',
	{
		...
		if isUsing then
			isUsing = false
			TriggerServerEvent('exile:setUsed', 'Weapons', 'society_'..PlayerData.thirdjob.name, false)
		end
	end)
end
...
function OpenCloakroomMenu(organization)
	...
	ESX.TriggerServerCallback('exile_organisations:getLicenses', function(licenses)
		...
		if licenses.addoncloakroom == 1 then
			...
			if PlayerData.thirdjob.grade >= 3 then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory',
		{
			...
			if data.current.value == 'Koszulka' then
				for k, v in pairs(Config.Organizacje) do
					if PlayerData and (PlayerData.thirdjob and PlayerData.thirdjob.name == v.praca) then
						if v.Tshirt or v.Tshirtdrugi then
							local tshirt = {}
							if PlayerData and (PlayerData.thirdjob and PlayerData.thirdjob.name == 'org22') then
								TriggerEvent('skinchanger:getSkin', function(skin)
									if skin.sex == 0 then
										tshirt = {
			...
			if data.current.value == 'skin_menu' then
				TriggerEvent('exile_organisations:openSkinMenu')
			end
...
function OpenBossMenu(organization, grade)
	ESX.TriggerServerCallback('exile_organisations:getLicenses', function(licenses)
		if licenses.safe == 1 then
			if PlayerData.thirdjob.grade >= grade then
				TriggerEvent('esx_society:openThirdBossMenu', organization, licenses.level, function(data, menu)
					menu.close()
...
CreateThread(function()
	while PlayerData.thirdjob == nil do
		Citizen.Wait(500)
	end
	while true do
		Citizen.Wait(30)
		if car then
			if PlayerData ~= nil and Config.DriveByList[PlayerData.thirdjob.name] and GetEntitySpeed(car) * 3.6 < 75 then
...
RegisterNetEvent("exile_organisations:openSkinMenu")
AddEventHandler("exile_organisations:openSkinMenu", function()
...
local coords = {
	...
	vector3(1446.45, 1153.68, 114.33),
	vector3(2527.42, 2586.28, 37.94),
...
CreateThread(function()
	...
	npc = CreatePed(5, model, 2527.42, 2586.28, 37.14, 0.0, false, true)
	npc2 = CreatePed(5, model, 1446.45, 1153.68, 113.43, 0.0, false, true)
	...
	SetEntityHeading(npc, 264.0)
	SetEntityHeading(npc2, 10.0)
...
function OpenKitsMenu()
	local elements = {}
	if PlayerData.thirdjob.name ~= nil and (PlayerData.thirdjob.grade_name == "szef" or PlayerData.thirdjob.grade >= 5) then
...
OpenNarkoMenu = function()
	TriggerEvent('falszywyy_barylki:mieszaczmenu', source)
end