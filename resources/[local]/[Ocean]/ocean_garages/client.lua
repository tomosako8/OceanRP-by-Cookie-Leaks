	...
	if IsImpounding == false then
		...
		ESX.TriggerServerCallback('flux_garages:checkMoney', function(hasMoney)
			if hasMoney == 1 then
				...
				SetNuiFocus(false)
				SendNUIMessage({openList = false})
				exports['OceanRP']:DrawProcent(100, function()
	...
	while true do
		...
		if (garage ~= nil) then
			...
			elseif garage.role then
				if PlayerData.job.name == garage.role or PlayerData.secondjob.name == garage.role or PlayerData.thirdjob.name == garage.role or string.sub(PlayerData.job.name, 4) == garage.role then
					if garage.vdist < 3.5 and not onNui then
						ESX.ShowHelpNotification('~w~Naciśnij ~INPUT_CONTEXT~ aby skorzystać ~y~z garażu\n~w~Naciśnij ~INPUT_THROW_GRENADE~ aby sprawdzić ~y~stan pojazdów')
	...
	while true do
		...
		if (zone ~= nil) then			
			if zone.marker == 1 then
				...
				if (zone.name == 'impoundpd' and PlayerData ~= nil and PlayerData.job ~= nil and (PlayerData.job.name == 'police' or PlayerData.job.name == 'sheriff')) or zone.name ~= 'impoundpd' then
	...
	if GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
		ESX.TriggerServerCallback('flux_garages:checkCar', function(kalbak)
			if kalbak == 1 or kalbak == 2 then
				vehProperties["fuelLevel"] = exports["LegacyFuel"]:GetFuel(veh)
		...
		for i=1, #vehicles, 1 do
			...
			TriggerEvent('esx_vehicleshop:getVehicles', function(base)
				...
				SendNUIMessage({
					...
					fuel = vehicles[i].fuelLevel and math.floor(vehicles[i].fuelLevel) or '??',
				})							  
			end)
		end
							...
							elseif data2.current.value == 'manage_sub' then
								...
								ESX.TriggerServerCallback('flux_garages:getSubOwners', function(dane, val)
									...
									ESX.TriggerServerCallback('flux_garages:getSubOwners2', function(danes, vals)
										...
										ESX.TriggerServerCallback('flux_garages:getSubOwners3', function(danet, valt)
											if danet ~= nil then
										...
										ESX.UI.Menu.Open(
											...
											function(data3, menu3)
												...
												if data3.current.h == 'third' then
													TriggerServerEvent('flux_garages:deleteSubowner3', plate, data3.current.value)
												end
												menu3.close()
											end,
											function(data3, menu3)
