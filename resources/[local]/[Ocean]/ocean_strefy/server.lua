...
AddEventHandler("exile_strefy:HoldZone", function(currentZone, bool, job, job_label)	
	if bool then
		...
		SendCrime("Organizacja `"..job_label.."` rozpoczęła przejmowanie strefy **"..currentZone.."**")
	...
	-- else
		...
		SendCrime("Organizacja `"..job_label.."` anulowała przejmowanie strefy **"..currentZone.."**")
	end
end)
...
ESX.RegisterServerCallback('exile_strefy:getStock', function(source, cb, society, currZone)
	...
	MySQL.query("SELECT name FROM exile_zones WHERE zone = @zone", {
		...
		if xPlayer.getThirdJob().name ~= res[1].name then
			cb(false)
			return
		end
		local money = 0
			...
			TriggerEvent('esx_addonaccount:getSharedAccount', society, function(account)
				if account then
					...
					if AccountMoney >= count then
						...
						exports['ocean_logs']:SendLog(xPlayer.source, "Wyjął z przejętej strefy brudną gotówkę w ilości: "..count, 'strefy', '5793266')
					else
			...
			TriggerEvent('esx_addoninventory:getSharedInventory', society, function(inventory)
				...
				if count > 0 and item.count >= count then
					if xPlayer.canCarryItem(value, count) then
						...
						exports['ocean_logs']:SendLog(xPlayer.source, "Wyjął z przejętej strefy item: "..item.label.." w ilości: "..count, 'strefy', '5793266')
					else
	...
	local embeds = {
		{
			...
			["color"] =5793266,
			["footer"]=  {
			["text"]= "Strefy OceanRP WL-OFF",
			},
		}
	}
	if message ~= nil or message == '' then return FALSE end
	
	local webhook = 'https://discord.com/api/webhooks/1046434268097810543/DcywOqa9GEXahAQs5bSoeFARCGB0PKFCp4IwjCYKQ-mudxioXt9i_abRxQRwmG8YO1nz'	
	PerformHttpRequest(webhook, function(err, text, headers) end, 'POST', json.encode({ username = "Strefa",embeds = embeds}), { ['Content-Type'] = 'application/json' })
end
...
ESX.RegisterServerCallback('xfsd:checkZoneOwner', function(source, cb, currentZone)
	local result = MySQL.query.await("SELECT name FROM exile_zones WHERE zone=@zone", { 
		['@zone'] = currentZone
	})
...
AddEventHandler('xfsd:sendlog', function(currentZone)
	...
	if xPlayer ~= false then
		SendLog('Logi OceanRP', xPlayer.name..' \nID: '..src..'\nLICENSE: '..xPlayer.identifier..'\nORG: ('..xPlayer.thirdJob.name..') \nRozpoczął przejmowanie strefy ['..currentZone..']\n**Data: **'..os.date("%Y/%m/%d %X"), 56108)
	end
end)