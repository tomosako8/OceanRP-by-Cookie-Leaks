...
AddEventHandler('esx:setThirdJob', function(thirdJob)
	TriggerServerEvent("exile_strefy:SetJob", thirdJob.name)
...
AddEventHandler('exile_strefy:refreshOcupped', function(org, currentZone)
	if PlayerData.thirdJob and PlayerData.thirdJob.name ~= nil then
		if PlayerData.thirdJob.name == org then
	...
	if PlayerData.thirdJob and PlayerData.thirdJob.name ~= nil then
		ESX.TriggerServerCallback('exile_strefy:checkStrefy', function(data)		
			for k,v in pairs(data) do
				if v[2] == PlayerData.thirdJob.name then
	...
	if PlayerData.thirdJob and PlayerData.thirdJob.name ~= nil then
		for i=1, #Config.Strefy, 1 do
			for ii,v in ipairs(Config.Strefy[i].Jobs) do
				if PlayerData.thirdJob.name == v then
...
AddEventHandler("exile_strefy:zoneTaken", function(currentZone)
	...
	TriggerServerEvent("exile_strefy:zoneTakenServer", PlayerData.thirdJob.name, PlayerData.thirdJob.label, currentZone)
end)
...
AddEventHandler("exile_strefy:RemoveActiveZone", function(currentZone, job_label, job)	
	if PlayerData.thirdJob ~= nil and PlayerData.thirdJob.name ~= nil then		
...
AddEventHandler("exile_strefy:startZone", function(currentZone, job, job_label)
	if PlayerData.thirdJob ~= nil and string.find(PlayerData.thirdJob.name, "org") then
		taking = true		
		--for i,v in ipairs(Config.Strefy[currentZone].Jobs) do
		for i = 12, 1, -1 do
			if not taking then
    ...
    while true do
        ...
        for i=1, #Config.Strefy, 1 do
			if PlayerData.thirdJob then
				if PlayerData.thirdJob.name ~= nil then
					for ii,v in ipairs(Config.Strefy[i].Jobs) do
						if PlayerData.thirdJob.name == v then
									...
									if not isDead and not IsPedInAnyVehicle(PlayerPedId(), false) then
										...
										if IsControlJustPressed(0, 38) then
											if not IsPedInAnyVehicle(PlayerPedId(), false) then
												...
												if not Config.Strefy[i].ocupped and not sWait and not Timer[currentZone] then
													sWait = true
													ESX.TriggerServerCallback('exile_strefy:CheckZone', function(hold)
														if not hold then
															TriggerServerEvent('exile_strefy:HoldZone', currentZone, true, PlayerData.thirdJob.name, PlayerData.thirdJob.label)
															...
															taskbarId = exports["ocean_taskbar"]:cancellableTaskBar(Config.secondsRemaining*1000, "Przejmowanie "..Config.Strefy[i].label, false)
													...
													if PlayerData.thirdJob.grade >= 2 then
							...
							if (isDead and Timer[currentZone]) or (Timer[currentZone] and IsPedInAnyVehicle(PlayerPedId(), false)) then
								...
								exports["ocean_taskbar"]:cancelTaskBar(taskbarId)
								Debilktoryrobiltenskryptjestzyciowymnieudacznikiemigejemigranawlon = false
								sWait = false
								TriggerServerEvent('exile_strefy:HoldZone', currentZone, false, PlayerData.thirdJob.name, PlayerData.thirdJob.label)
