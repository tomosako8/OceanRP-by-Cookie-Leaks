...
admins = {
    'steam:110000148919c3c', -- bEZ NIKU
}
...
ESX.RegisterServerCallback('esx_misiaczek:getVehicleFromPlate', function(source, cb, plate)
	MySQL.query('SELECT owner, digit FROM owned_vehicles WHERE plate = ?', { plate }, function(result)
		if result[1] ~= nil then
            ...
            local x = exports["OceanRP"]:getPolicePlate(plate)
            if x ~= nil then
                local data = {
					owner = x,
					police = true
...
AddEventHandler('ExileRP:saveCours', function(secondjob, secondjob_grade, source)
    local _source = source
	local identifier = ESX.GetPlayerFromId(_source).identifier
    local xPlayer = ESX.GetPlayerFromId(_source)
    local result = MySQL.query.await("SELECT courses_count FROM user_courses WHERE identifier = ? AND digit = ? AND secondjob = ?", {identifier, xPlayer.getDigit(), secondjob})
    if result[1] == nil then
        MySQL.update('INSERT INTO user_courses (identifier, firstname, lastname, digit, courses_count, secondjob, secondjob_grade) VALUES (?, ?, ?, ?, ?, ?, ?)', {identifier, xPlayer.character.firstname, xPlayer.character.lastname, xPlayer.getDigit(), 1, secondjob, secondjob_grade})
    else
        local kursCount = result[1].courses_count
        MySQL.update('UPDATE user_courses SET courses_count = ?, secondjob_grade = ? WHERE identifier = ? AND digit = ? and secondjob = ?', {identifier, xPlayer.getDigit(), kursCount + 1, secondjob_grade, secondjob})
        exports['esx_oceanmenu']:KursyChange(_source, kursCount)
    end
end)

ESX.RegisterServerCallback('ExileRP:getCourses', function(source, cb, secondjob)
	MySQL.query('SELECT secondjob_grade, firstname, lastname, courses_count FROM user_courses WHERE secondjob = ? ORDER BY secondjob_grade DESC', {secondjob}, function (results)
...
AddEventHandler('ExileRP:saveCours', function(job, job_grade, source)
	...
	local identifier = ESX.GetPlayerFromId(_source).identifier
    local xPlayer = ESX.GetPlayerFromId(_source)
    local result = MySQL.query.await("SELECT courses_count, courses_count_secound FROM user_courses WHERE identifier = @identifier AND digit = @digit AND secondjob = @job", {
...
ESX.RegisterServerCallback('ExileRP:getCourses', function(source, cb, job)
	MySQL.query('SELECT secondjob_grade, firstname, lastname, courses_count FROM user_courses WHERE secondjob = @job ORDER BY secondjob_grade DESC', {
		['@job'] = job,
	}, function (results)
		if results[1] ~= nil then
			cb(results)
