local Beds = {
	...
	['Pillbox2'] = {
		{
			Position = vector4(325.98, -574.5, 43.60, 342.97),
			GetUp = vector4(344.02, -587.12, 27.95, 293.65),
			Occupied = false
		},
		{
			Position = vector4(323.10, -573.17, 43.60, 342.97),
			GetUp = vector4(344.02, -587.12, 27.95, 293.65),
			Occupied = false
		},
	},
...
AddEventHandler('Exile:Barabasz', function(ems, zone, accountType)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local HasInsurance = exports['esx_oceanmenu']:CheckInsuranceEMS(xPlayer.job.name)
			...
			if has or HasInsurance then
				...
				if accountType == "money" then
					if xPlayer.getMoney() >= 7500 then
						StartTreatment(_source, zone, 7500, accountType)
					else
						xPlayer.showNotification("~r~Nie posiadasz ~g~500$~r~ aby zapłacić za ~g~pomoc medyczną")
					end
				elseif accountType == "bank" then
					if xPlayer.getAccount('bank').money >= 7500 then
						StartTreatment(_source, zone, 7500, accountType)
					else
