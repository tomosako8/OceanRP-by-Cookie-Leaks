        ...
        MySQL.single('SELECT * FROM jakastabela WHERE hex = ?', {hex}, function(result)
            if result ~= nil then --Czy użytkownik jest w tabeli
                if result.time <= os.time() then
                    ...
                    DajNagrode(source)
                    exports['ocean_logs']:SendLog(source, "UDANE: Otrzymał nagrodę za posiadanie w nicku steam odpowiedniego tagu w wysokości: 15.000$", 'nagrodynick', '5793266')
                else
            ...
            if string.find(hex, "steam:110000") then --czy dobrze wpisał hexa
                ...
                MySQL.single('SELECT * FROM battlepass WHERE hex = ?', {hex}, function(result)
                    if result == nil then -- czy niema battle pasa
                        ...
                        exports['ocean_logs']:SendLog(source, "DODANO: Battlepass dla: "..hex, 'battlepass', '5793266')
                    else
            ...
            if string.find(hex, "steam:110000") then --czy dobrze wpisał hexa
                ...
                MySQL.single('SELECT * FROM battlepass WHERE hex = ?', {hex}, function(result)
                    if result ~= nil then -- czy ma battle pasa
                        MySQL.update('DELETE FROM battlepass WHERE hex = ? ', {hex}, function(id)
                            --print(id)
                        end)
                        TriggerClientEvent("esx:showNotification", source, "Zabrano Battlepass dla: "..hex)
                        exports['ocean_logs']:SendLog(source, "ZABRANO: Battlepass dla: "..hex, 'battlepass', '5793266')
                    else
    ...
    MySQL.single('SELECT * FROM battlepass WHERE hex = ?', {hex}, function(result)
        if result ~= nil then --Czy użytkownik ma battlepassa
            if result.time <= os.time() then
                ...
                exports['ocean_logs']:SendLog(source, "ODEBRAL: Battlepass", 'battlepass', '5793266')
