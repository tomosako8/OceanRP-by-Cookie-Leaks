    ...
    if not Radio.On then
        if radioConfig.Frequency.Access[radioConfig.Frequency.Current] then
            ...
            for i=1, #radioConfig.Frequency.Access[radioConfig.Frequency.Current], 1 do
                if PlayerData.job.name == radioConfig.Frequency.Access[radioConfig.Frequency.Current][i] or PlayerData.thirdjob.name == radioConfig.Frequency.Access[radioConfig.Frequency.Current][i] then
                    found = true
                    break
                end
            end
...
function Radio:Add(id)
    ...
    cacheOne = {}
    if Radio.On then
        TriggerServerEvent("exilerp:registerChannel", id)
    end
...
function Radio:Remove()
    cache = {}
    cacheOne = {}
    TriggerServerEvent("exilerp:unregisterChannel")
...
RegisterNetEvent("exilerp:returnOrg", function(orgs) 
    for i,v in ipairs(orgs) do
        table.insert(CustomLabels, {v.name, v.label})
    end
end)
CreateThread(function() 
    TriggerServerEvent("exilerp:fetchOrganizations")
end)
                        ...
                        if not radioConfig.Frequency.Private[input] or radioConfig.Frequency.Access[input] then
                            ...
                            if radioConfig.Frequency.Access[input] then
                                for i=1, #radioConfig.Frequency.Access[input], 1 do
                                    if PlayerData.job.name == radioConfig.Frequency.Access[input][i] or PlayerData.thirdjob.name == radioConfig.Frequency.Access[input][i] then
                                        found = true
                                        break
                                    end
                                end   
...
RegisterNetEvent("exilerp:openCrimeRadio", function(list) 
...
function OpenRadioList() 
    ...
    ESX.UI.Menu.Open('list', GetCurrentResourceName(), 'radio_list', elements, function(data, menu)
        ...
        if data.value == 'kick' then
            menu.close()
            TriggerServerEvent("exilerp:kickFromRadio", employee.id)
        ...
        elseif data.value == "move" then
            ...
            ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'radio_moving', {
                ...
                TriggerServerEvent("exilerp:moveInRadio", employee.id, value)
...
function RadioList(args) 
    if args[1] then
        ...
        if radio and radio ~= nil and radio > 0 and radio < 1000 then
            TriggerServerEvent("exilerp:openRadioListS", radio)
            return
        elseif args[1] == "all" then
            TriggerServerEvent("exilerp:openRadioListS", "all")
            return
        end    
    end
end

radiolistpd = function()
...
radiolistpdhc = function()
    if (PlayerData.job.name == 'police' or PlayerData.job.name == 'sheriff') and PlayerData.job.grade >= 8 then
        OpenRadioList()
    else
        ESX.ShowNotification('Nie posiadasz do tego dostępu!')
    end
...
RegisterNetEvent("exilerp:moveTo", function(channel) 
