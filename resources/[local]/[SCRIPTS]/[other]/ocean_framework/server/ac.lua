...
local banning = {}
RegisterServerEvent("exilerp:banPlr", function(id, d, rsn) 
    local src = source
    if tonumber(src) ~= nil then
      TriggerEvent("exilerp:banPlr", "nigger", src, "Tried to ban player (OceanAC)")
    else
        src = d
        ...
        if tonumber(src) ~= nil or tonumber(src) ~= 0 then  
            ...
            MySQL.update("UPDATE exile_bans SET reason=@reason, expired=@expired, bannedby=@bannedby, isBanned=1 WHERE license=@license", {
                ...
                local tekst = string.format("```\nKto: %s\nPrzez: Ocean Anticheat\nPowód: Cheating [%s]\nData: %s\nDane: \n%s\nSerwer: %s\n```", targetName, reason, date, daneString, "WL OFF")
                    
                PerformHttpRequest(Config.BanWebhook, function(err, text, headers) end, "POST", json.encode({username = "OceanRP", content = tekst}), { ["Content-Type"] = "application/json" })
				PerformHttpRequest(Config.BanWebhookLogs, function(err, text, headers) end, "POST", json.encode({username = "OceanRP", content = tekst}), { ["Content-Type"] = "application/json" })
                banning[src] = false
            end)
            TriggerClientEvent("exilerp:crash", src)
			DropPlayer(src, "Zostałeś zbanowany permanentnie na tym serwerze przez Ocean Anticheat.")
        end
    end    
end)

RegisterServerEvent("exilerp:gotMeDMG", function(z) 
	local src = source
	TriggerEvent("exilerp:banPlr", "nigger", src, "Detection: "..z.." (Mods)")
end)

local payCheckStrikes = {}
local payCheckEvent = "exilerp:payCheck"
RegisterServerEvent(payCheckEvent, function(id) 
	local src = source
	if tonumber(src) ~= nil then
		TriggerEvent("exilerp:banPlr", "nigger", src, "Detection: Event "..payCheckEvent.." detected")
	else
		if payCheckStrikes[id] == nil then
			...
			if strikes > 5 then
				TriggerEvent("exilerp:banPlr", "nigger", id, "Detection: Paycheck spam detected, `"..strikes.."` in a row")
			else
...
--[[AddEventHandler('explosionEvent', function(source, ev)
	...
	if ev and ev.explosionType and ev.damageScale ~= 0.0 and ev.ownerNetId == 0 and ev.explosionType ~= 13 then
		exports['ocean_logs']:SendLog(src, "Próbował wywołać eksplozję: `"..ev.explosionType.."`", 'anticheat')
	end
	CancelEvent()
end)]]

local whTimeout = {}
RegisterNetEvent("exilerp:sendScreen", function(ss, key, count) 
	local src = source
	...
	if string.find(ss, "cdn.discordapp.com") or string.find(ss, "cdn.discord.com") then
		...
		print("^3[OceanRP] ^7Sending ^3INSERT^7 webhook for player ^3"..GetPlayerName(src).."^7")
		PerformHttpRequest(Config.InsertWebhook, function(err, text, headers) end, "POST", json.encode({username = "oceanrp Inserts", embeds = embed}), { ["Content-Type"] = "application/json" })
		whTimeout[src] = true
...
AddEventHandler("exile:sendnibba", function(type, arg, skrypt)
	...
	if not doesPlayerHavePerms(_source, type) then
		TriggerEvent("exilerp:banPlr", "nigger", _source, string.format("Injection in script %s (OceanRP-Handler)", skrypt))
	end
end)
...
AddEventHandler("playerConnecting", function(playerName, setKickReason)
	for name in pairs(forbiddenNames) do
		if(string.gsub(string.gsub(string.gsub(string.gsub(playerName:lower(), "-", ""), ",", ""), "%.", ""), " ", ""):find(forbiddenNames[name])) then
			setKickReason("\n[OceanRP AC] Twój nick jest niedozwolony, aby zagrać na serwerze zmień nick na poprawny!")
			CancelEvent()
			print("^3[OceanRP] "..playerName.."^7 was kicked for blacklisted name "..forbiddenNames[name])
			break
		end
	end
end)
	...
	if data.weaponType == 911657153 or data.weaponType == 1171102963 then
		...
		if tazedata[src] then
			if tazedata[src].count > 10 then
				...
				if isSpamTaze then
					CancelEvent()
					TriggerEvent(`exilerp:banPlr`, "nigger", src, "Detection: Tazed players "..tazedata[src].count.." times in 10 seconds")
				end
			end
		end
	end
