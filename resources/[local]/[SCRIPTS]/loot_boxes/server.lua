...
AddEventHandler('exile_boxes:giveReward', function (t, data, amount)
	...
	if t == "item" then
		...
		exports['ocean_logs']:SendLog(source, "Wylosowal "..data.." x"..amount.."", 'skrzynki', '5793266')
	elseif t == "weapon" then
		xPlayer.addInventoryWeapon(data, amount, 1, false)
		exports['ocean_logs']:SendLog(source, "Wylosowal "..data.." x"..amount.." z 5 ammo", 'skrzynki', '5793266')
	elseif t == "money" then
		xPlayer.addMoney(data)
		exports['ocean_logs']:SendLog(source, "Wylosowal "..data.."$", 'skrzynki', '5793266')
	elseif t == "black_money" then
		xPlayer.addAccountMoney('black_money', data)
		exports['ocean_logs']:SendLog(source, "Wylosowal "..data.."$ brudnej gotowki", 'skrzynki', '5793266')
	end
end)]]
...
RegisterServerEvent('exile_boxes:giveReward', function() 
	...
	if not playerWinnings[src] then
		TriggerEvent('exilerp:banPlr', "nigger", src, "Próba oszukiwania w skrzynkach")
		return
	end
	...
	if win.item then
		...
		exports['ocean_logs']:SendLog(src, "Wylosowal "..win.item.." x"..win.amount.."", 'skrzynki', '5793266')
	elseif win.weapon then
		xPlayer.addInventoryItem(win.weapon, win.amount, 1, false)
		exports['ocean_logs']:SendLog(src, "Wylosowal "..win.weapon.." x"..win.amount.." z 5 ammo", 'skrzynki', '5793266')
	elseif win.money then
		xPlayer.addMoney(win.money)
		exports['ocean_logs']:SendLog(src, "Wylosowal "..win.money.."$", 'skrzynki', '5793266')
	elseif win.black_money then
		xPlayer.addAccountMoney('black_money', win.black_money)
		exports['ocean_logs']:SendLog(src, "Wylosowal "..win.black_money.."$ brudnej gotowki", 'skrzynki', '5793266')
	end
	TriggerClientEvent("exilerp:save", src)
...
AddEventHandler("exile_boxes:exchange", function(type)
	local _source = source
	TriggerEvent('exilerp:banPlr', "nigger", _source, "Próba wymienienia serc na skrzynki :kekw:")
end)