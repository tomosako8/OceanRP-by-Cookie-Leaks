	...
	if zone == 'GunShop' then
		if type == 1 then
			if xPlayer.getMoney() >= selectedWeapon.price then
				...
				exports['ocean_logs']:SendLog(source, "Zakupił broń w sklepie: "..weaponName, 'sklepzbronia')
			end
		elseif type == 2 then			
			if xPlayer.getMoney() >= selectedWeapon.price then
	...
	elseif zone == 'GunShopDS' then
			if type == 1 then
				if xPlayer.getAccount('black_money').money >= selectedWeapon.price then
					...
					xPlayer.addInventoryItem(weaponName, 1)
					exports['ocean_logs']:SendLog(source, "MAFIA: Zakupił broń w sklepie: "..weaponName, 'sklepzbronia')
				end
			elseif type == 2 then			
				if xPlayer.getAccount('black_money').money >= selectedWeapon.price then
					if xPlayer.canCarryItem(weaponName, 1) then
