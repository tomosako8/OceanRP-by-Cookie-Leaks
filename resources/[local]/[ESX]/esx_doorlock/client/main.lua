		...
		for i=1, #DoorList do
			...
			if distance < maxDistance then
				...
				if isAuthorized then
					DrawText3Ds(doorID.textCoords.x, doorID.textCoords.y, doorID.textCoords.z, (doorID.locked and '🔒' or '🔓'))
				end
...
function IsAuthorized(doorID)
	...
	if doorID.authorizedJobs then
		for i=1, #doorID.authorizedJobs, 1 do 
			if ESX.PlayerData.job.name == doorID.authorizedJobs[i] or string.sub(ESX.PlayerData.job.name, 4) == doorID.authorizedJobs[i] or ESX.PlayerData.thirdjob.name == doorID.authorizedJobs[i] or Properties[doorID.authorizedJobs[i]] then
				return true
			end
		end
	end
...
AddEventHandler('esx_doorlock:setState', function(doorID, state)
	...
	if DoorList[doorID] then
		ApplyDoorState(DoorList[doorID])
