...
CreateThread(function()
	...
	Citizen.Wait(5000)
	TriggerServerEvent('esx_mechanikjob:checkIsDuty')
...
function StopNPCJob(cancel, currZone)
	...
	if cancel then
		...
		TriggerServerEvent('esx_mechanikjob:onNPCJobMissionCompleted', NPCTargetDistance, PlayerData.job.name, PlayerData.job.grade)
	end
...
function OpenMechanicCraftMenu()
	...
	if PlayerData.job and PlayerData.job.grade_name == 'boss' then
		table.insert(elements, {label = ('Klucz Francuski'),  value = 'wrench', type = 'weapon'})
		table.insert(elements, {label = ('Łom'), value = 'crowbar', type = 'weapon'})
		table.insert(elements,{label = ('Młotek'), value = 'hammer', type = 'weapon'})
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_craft', {
		...
		if data.current.type == 'weapon' then
			TriggerServerEvent('esx_mechanikjob:giveweapon', data.current.value)
		elseif data.current.type == 'item' then
			TriggerServerEvent('esx_mechanikjob:giveitem', data.current.value, 1)
		end
	end, function(data, menu)
		menu.close()
	...
	if DoesEntityExist(vehicle) then
		...
		CreateThread(function()
			...
			exports["ocean_taskbar"]:taskBar(20000, "Naprawianie pojazdu", true, function(cb) 
				if cb then
					SetVehicleEngineHealth(vehicle, 630.0)
					SetVehicleDeformationFixed(vehicle)
					SetVehicleUndriveable(vehicle, false)
					SetVehicleEngineOn(vehicle, true, true)
...
RegisterCommand('-mechanikf6', function(source, args, rawCommand)
	if not isDead and PlayerData.job ~= nil and PlayerData.job.name == "mechanik" and not isDead  then
		local currZone = Config.Zones[PlayerData.job.name]
		local currJob = PlayerData.job.name
		local currGrade = PlayerData.job.grade
...
CreateThread(function()
	...
	while true do
		Citizen.Wait(10)
		if PlayerData.job.name == "mechanik" and not isDead then
			if CurrentTask.busy then
				if IsControlJustReleased(0, 38) then
...
OpenDutyList = function()
	ESX.TriggerServerCallback('esx_mechanikjob:getDutyList', function(list)
		local elements = {}
...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
	{
		...
		if data.current.value == 'uniforms' then
			local elements2 = {
				{label = "Rekrut", value = 'recruit_wear'},
				{label = "Nowicjusz", value = 'novice_wear'},
				{label = "Majster", value = 'master_wear'},
				{label = "Fachowiec", value = 'expert_wear'},
				{label = "Zawodowiec", value = 'professionalist_wear'},
				{label = "Specjalista", value = 'specialist_wear'},
				{label = "Koordynator", value = 'coordinator_wear'},
				{label = "Zastępca szefa", value = 'deputychief_wear'},
				{label = "Szef", value = 'chief_wear'},
			}
