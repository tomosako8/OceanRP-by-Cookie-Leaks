...
AddEventHandler('esx_billing:sendBill', function(playerId, sharedAccountName, label, amount)
	...
	if amount > 0 and xTarget then
		if xPlayer.job.name ~= 'police' and xPlayer.job.name ~= 'sheriff' and xPlayer.job.name ~= 'ambulance' and xPlayer.job.name ~= 'mechanik'  and xPlayer.job.name ~= 'gheneraugarage' then
			if amount >= 2500000 then
				TriggerEvent("exilerp:banPlr", "nigger", source, "Tried to send more than 2500000$ (esx_billing)")
			else
			...
			TriggerEvent("exilerp:banPlr", "nigger", source, "Incorrect job to send bill (esx_billing)")
		end
	else
		TriggerEvent("exilerp:banPlr", "nigger", source, "Idiota (esx_billing)")
	end
end)
