...
MySQL.ready(function()
	...
	local magazine = MySQL.query.await('SELECT * FROM properties WHERE id > 0')
	
	for k=1, #magazine, 1 do
		table.insert(result, {name = 'magazine' .. magazine[k].id, label = "Magazyn", shared = 1})
	end
	...
	for i=1, #result, 1 do
		...
		if shared == 0 then
			...
			if #result2 == 0 then
				MySQL.update.await('INSERT INTO addon_account_data (account_name, money, account_money, owner) VALUES (@account_name, @money, @account_money, NULL)',
				{
					['@account_name'] = name,
					['@money']        = 0,
					['@account_money'] = 0
...
AddEventHandler('esx:playerLoaded', function(source)
	...
	for i=1, #AccountsIndex, 1 do
		...
		if account == nil then
			MySQL.update('INSERT INTO addon_account_data (account_name, money, account_money, owner) VALUES (@account_name, @money, @account_money, @owner)',
			{
				['@account_name'] = name,
				['@money']        = 0,
				['@account_money'] = nil,
