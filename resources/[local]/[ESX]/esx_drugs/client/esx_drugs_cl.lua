...
CreateThread(function()
	...
	while true do
		...
		for k,v in pairs(Config.Zones) do
			if #(coords - vec3(v.x, v.y, v.z)) < Config.ZoneSize.x and not exports['esx_policejob']:IsCuffed() and not exports['ocean_trunk']:checkInTrunk() then
				isInMarker  = true
				currentZone = k
			end
		end
				...
				if IsControlJustReleased(0, Keys['E']) then
					ESX.TriggerServerCallback("esx_scoreboard:getConnectedCops", function(MisiaczekPlayers)
						if MisiaczekPlayers then
							if MisiaczekPlayers['players'] >= 1 then
								isInZone = true
