...
RegisterNetEvent('esx_sklepholdup:currentlyRobbing')
AddEventHandler('esx_sklepholdup:currentlyRobbing', function(currentStore)
...
RegisterNetEvent('esx_sklepholdup:killBlip')
AddEventHandler('esx_sklepholdup:killBlip', function()
	if blipRobbery ~= nil then
		RemoveBlip(blipRobbery)
	end
end)

RegisterNetEvent('esx_sklepholdup:setBlip')
AddEventHandler('esx_sklepholdup:setBlip', function(position, alert)
...
RegisterNetEvent('esx_sklepholdup:tooFar')
AddEventHandler('esx_sklepholdup:tooFar', function()
	holdingUp, store = false, ''
	exports["esx_oceanchat"]:cancelProgress()
	ESX.ShowNotification(_U('robbery_cancelled'))
end)

RegisterNetEvent('esx_sklepholdup:robberyComplete')
AddEventHandler('esx_sklepholdup:robberyComplete', function(award)
	holdingUp, store = false, ''
	Citizen.InvokeNative(0xAAA34F8A7CB32098, PlayerPedId())
	ESX.ShowAdvancedNotification('~b~~h~OBRABOWANIE UDANE!', 'Z Napadu na sklep zdobyto:', award)
end)

RegisterNetEvent('esx_sklepholdup:animation')
AddEventHandler('esx_sklepholdup:animation', function(code)
...
RegisterNetEvent('esx_sklepholdup:startTimer')
AddEventHandler('esx_sklepholdup:startTimer', function(timer)
	CreateThread(function()
		exports["esx_oceanchat"]:showProgress("notification_important", "top", "Rabowanie...", "Rozpoczęto napad", "grey-10", timer, true)
	...
	while true do
			...
			if distance < 10 then
				...
				if not holdingUp then
					...
					if distance < 1.25 then
						ESX.ShowHelpNotification('Naciśnij ~INPUT_CONTEXT~ aby rozpocząć ~r~obrabowywanie sklepu~s~')
						if IsControlJustReleased(0, Keys['E']) then
							if IsPedArmed(PlayerPedId(), 4) then
								TriggerServerEvent('esx_sklepholdup:robberyStarted', k)
		...
		if holdingUp then
			...
			if Vdist(playerPos.x, playerPos.y, playerPos.z, storePos.x, storePos.y, storePos.z) > Config.MaxDistance then
				sleep = false
				TriggerServerEvent('esx_sklepholdup:tooFar', store)
				Citizen.Wait(1000)
			end
		end
