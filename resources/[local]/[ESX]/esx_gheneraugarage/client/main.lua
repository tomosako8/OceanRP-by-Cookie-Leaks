...
CreateThread(function()
	...
	TriggerServerEvent('esx_gheneraugarage:checkIsDuty')
...
function StopNPCJob(cancel, currZone)
	...
	if cancel then
		...
		TriggerServerEvent('esx_gheneraugarage:onNPCJobMissionCompleted', NPCTargetDistance, PlayerData.job.name, PlayerData.job.grade)
	end
...
function OpenMechanicCraftMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_craft', {
		...
		if data.current.type == 'weapon' then
			TriggerServerEvent('esx_gheneraugarage:giveweapon', data.current.value)
		elseif data.current.type == 'item' then
			TriggerServerEvent('esx_gheneraugarage:giveitem', data.current.value, 1)
		end
	end, function(data, menu)
		menu.close()
			...
			if czesci >= 3 then
				...
				if DoesEntityExist(vehicle) then
					...
					CreateThread(function()
						...
						exports["ocean_taskbar"]:taskBar(20000, "Naprawianie silnika", true, function(cb) 
							if cb then
								local first = true
...
AddEventHandler('esx_gheneraugarage:onFixkitFree', function()
	...
	if DoesEntityExist(vehicle) then
		...
		CreateThread(function()
			...
			exports["ocean_taskbar"]:taskBar(20000, "Naprawianie pojazdu", true, function(cb) 
				if cb then
					SetVehicleEngineHealth(vehicle, 1000.0)
					SetVehicleDeformationFixed(vehicle)
					SetVehicleUndriveable(vehicle, false)
...
function UseTheJackFunction(vehicle)
	...
	if GetEntityModel(vehicle) == GetEntityModel(npcVeh) then
		if not canCarJack then
			...
			carJackPulled = true
			exports["esx_oceanchat"]:updateSolid(progressID, "record_voice_over", "center-special", "Zlecenie", "Napraw pojazd", "positive")
		end
	end
	...
	exports["ocean_taskbar"]:cancellableTaskBar(6000, label, false)
	-- Raise Jack Task:
          ...
          if DoesEntityExist(vehicle) then
			...
			if not IsThisModelAHeli(model) and not IsThisModelAPlane(model) and not IsThisModelABoat(model) then
              ...
              CreateThread(function()				
				...
				exports["ocean_taskbar"]:taskBar(10000, "Trwa odblokowywanie", true, function(cb) 
					if cb then
      ...
      elseif data.current.value == 'fix_vehicle' then
		...
		if DoesEntityExist(vehicle) then
			...
			CreateThread(function()
				...
				exports["ocean_taskbar"]:taskBar(17000, "Naprawianie silnika", true, function(cb) 
					if cb then
						SetVehicleEngineHealth(vehicle, 1000.0)
						SetVehicleDeformationFixed(vehicle)
						SetVehicleUndriveable(vehicle, false)
	  ...
	  elseif data.current.value == 'fix_body' then
		...
		if DoesEntityExist(vehicle) then
			...
			CreateThread(function()
				...
				exports["ocean_taskbar"]:taskBar(length, "Naprawianie karoserii", true, function(cb) 
					if cb then
						SetVehicleFixed(vehicle)
						SetVehicleDeformationFixed(vehicle)
						SetVehicleUndriveable(vehicle, false)
      ...
      elseif data.current.value == 'clean_vehicle' then
		...
		if DoesEntityExist(vehicle) then
			...
			CreateThread(function()
				...
				exports["ocean_taskbar"]:taskBar(10000, "Mycie pojazdu", true, function(cb) 
					if cb then
...
AddEventHandler('esx_gheneraugarage:hasExitedMarker', function(zone)
	if zone =='VehicleDelivery' then
		NPCTargetDeleterZone = false
	elseif zone == 'Craft' then
		TriggerServerEvent('esx_gheneraugarage:stopCraft')
		TriggerServerEvent('esx_gheneraugarage:stopCraft2')
		TriggerServerEvent('esx_gheneraugarage:stopCraft3')
	end
...
CreateThread(function()
	...
	while true do
		Citizen.Wait(3)
		if PlayerData.job.name == "gheneraugarage" then
			local sleep = true
...
CreateThread(function()
	...
	while true do
		Citizen.Wait(3)
		if PlayerData.job.name == "gheneraugarage" or PlayerData.job.name == "offgheneraugarage" then
...
RegisterCommand('-ghenerauf6', function(source, args, rawCommand)
	if not isDead and PlayerData.job ~= nil and PlayerData.job.name == "gheneraugarage" and not isDead  then
		local currZone = Config.Zones[PlayerData.job.name]
		local currJob = PlayerData.job.name
		local currGrade = PlayerData.job.grade
...
RegisterKeyMapping('-ghenerauf6', 'Włącz/wyłącz menu (Ghenerau Garage)', 'keyboard', 'F6')
...
RegisterKeyMapping('-ghenerauf9', 'Zacznij pracować (Ghenerau Garage)', 'keyboard', 'F9')
...
CreateThread(function()
	...
	while true do
		Citizen.Wait(10)
		if PlayerData.job.name == "gheneraugarage" and not isDead then
			if CurrentTask.busy then
				if IsControlJustReleased(0, 38) then
...
OpenDutyList = function()
	ESX.TriggerServerCallback('esx_gheneraugarage:getDutyList', function(list)
		local elements = {}
...
function TakeJob(x) 
	...
	if PlayerData.job.name == 'gheneraugarage' or PlayerData.job.name == 'offgheneraugarage' then
		shownProgress = true
		exports["esx_oceanchat"]:showSolid(progressID, "hourglass_bottom", "center-special", "Zlecenie", "Oczekuj na informację od klienta...", "warning")
		Citizen.SetTimeout(3000, function() 
			...
			exports["esx_oceanchat"]:updateSolid(progressID, "gps_fixed", "center-special", "Zlecenie", "Dojedź na miejsce klienta", "positive")
			PrepareJob()
			...
			if #(blipCoords - playerCoords) < 20.0 then
				if playerVehicle then
					if GetEntityModel(playerVehicle) == GetHashKey("lsc_ford150") then
						RemoveBlip(currentBlip)
						exports["esx_oceanchat"]:updateSolid(progressID, "record_voice_over", "center-special", "Zlecenie", "Porozmawiaj z klientem", "positive")
						canTalkWithNPC = true
						part1 = true
					else
...
function Dialog1() 
	...
	exports["esx_oceanchat"]:updateSolid(progressID, "record_voice_over", "center-special", "Klient", "Witam, dziękuje za szybki przyjazd.", "black")
	Citizen.Wait(3000)
	ClearPedTasksImmediately(npc)
	TaskPlayAnim(npc, anims.talking2.lib, anims.talking2.anim, 8.0, 0.0, -1, 1, 0, 0, 0, 0)
	exports["esx_oceanchat"]:updateSolid(progressID, "record_voice_over", "center-special", "Klient", "Jadąc wyjechał mi pojazd z nad przeciwka i wjechałem w drzewo.", "black")
	Citizen.Wait(3000)
	ClearPedTasksImmediately(npc)
	TaskPlayAnim(npc, anims.talking3.lib, anims.talking3.anim, 8.0, 0.0, -1, 1, 0, 0, 0, 0)
	exports["esx_oceanchat"]:updateSolid(progressID, "record_voice_over", "center-special", "Klient", "Stało się to szybko i nagle!", "black")
	Citizen.Wait(1500)
	ClearPedTasksImmediately(npc)
	TaskPlayAnim(npc, anims.talking4.lib, anims.talking4.anim, 8.0, 0.0, -1, 1, 0, 0, 0, 0)
	exports["esx_oceanchat"]:updateSolid(progressID, "record_voice_over", "center-special", "Klient", "Prosiłbym o naprawę pojazdu.", "black")
	Citizen.Wait(5000)
	...
	exports["esx_oceanchat"]:updateSolid(progressID, "precision_manufacturing", "center-special", "Zlecenie", "Podnieś pojazd na podnośnik i go napraw", "positive")
...
function Dialog2() 
	ClearPedTasksImmediately(npc)
	TaskPlayAnim(npc, anims.talking1.lib, anims.talking1.anim, 8.0, 0.0, -1, 1, 0, 0, 0, 0)
	exports["esx_oceanchat"]:updateSolid(progressID, "record_voice_over", "center-special", "Klient", "Dziękuje za naprawe! Oto twoja zapłata.", "black")
	Citizen.Wait(3000)
	SetCamActive(cam, false)
	RenderScriptCams(false, false, 500, false, false)
	DestroyCam(cam)
	...
	exports["esx_oceanchat"]:updateSolid(progressID, "task_alt", "center-special", "Zlecenie", "Wykonano zlecenie! Zarobiono "..taskCoords.price.."$", "positive")
	Citizen.Wait(15000)
	EndMission()
end

function EndMission() 
	exports["esx_oceanchat"]:hideSolid(progressID)
	RemoveBlip(currentBlip)
	if cam then
		SetCamActive(cam, false)
		RenderScriptCams(false, false, 500, false, false)
...
function VehicleFixed() 
	...
	exports["esx_oceanchat"]:updateSolid(progressID, "record_voice_over", "center-special", "Zlecenie", "Naprawiono pojazd. Porozmawiaj z klientem", "positive")
	canTalkWithNPC = true
	part2 = true
end
