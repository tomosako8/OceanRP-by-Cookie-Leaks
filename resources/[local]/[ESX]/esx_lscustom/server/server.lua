...
AddEventHandler('LSC:lock', function(b,garage)
	if b ~= nil then
		...
		TriggerClientEvent('LSC:lock', -1, occuped, 
			math.floor(exports['esx_scoreboard']:CounterPlayers('mechanik') + 
			exports['esx_scoreboard']:CounterPlayers('gheneraugarage')
			)
		)
	end
end)
...
ESX.RegisterServerCallback('esx_vehicleshop:getVehiclePrice', function(source, cb, model)
	MySQL.query('SELECT price FROM vehicles WHERE model = @model', {
		['@model'] = model
	}, function(result)
