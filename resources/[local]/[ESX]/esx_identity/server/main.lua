...
function getIdentity(source, callback)
	...
	MySQL.query('SELECT identifier, firstname, lastname, dateofbirth, sex, height FROM `users` WHERE `identifier` = ?', {identifier}, function(result)
	...
	MySQL.query('SELECT * FROM characters WHERE identifier = ?', {identifier}, function(result)
		...
		if #result == 1 then
			MySQL.update('UPDATE characters SET accounts = @accounts, skin = @skin, job = @job, job_grade = @job_grade, job_level = @job_level, job_id = @job_id, secondjob = @secondjob, secondjob_grade = @secondjob_grade, thirdjob = @thirdjob, thirdjob_grade = @thirdjob_grade, inventory = @inventory, firstname = @firstname, lastname = @lastname, dateofbirth = @dateofbirth, sex = @sex, height = @height, status = @status, isDead = @isDead, phone_number = @phone_number, account_number = @account_number, tattoos = @tattoos, position = @position WHERE identifier = @identifier and digit = @digit', {
				...
				if(checkAllow(data.firstname) and checkAllow(data.lastname)) then
...
function updateIdentity(identifier, data, callback)
	MySQL.update('UPDATE `users` SET `firstname` = @firstname, `lastname` = @lastname, `dateofbirth` = @dateofbirth, `sex` = @sex, `height` = @height WHERE identifier = @identifier', {
		['@identifier']		= identifier,
		['@firstname']		= data.firstname,
		['@lastname']		= data.lastname,
		['@dateofbirth']	= data.dateofbirth,
...
function deleteIdentity(identifier, data, callback)
	MySQL.update('DELETE FROM `characters` WHERE identifier = @identifier AND firstname = @firstname AND lastname = @lastname AND dateofbirth = @dateofbirth AND sex = @sex AND height = @height', {
		['@identifier']		= identifier,
		['@firstname']		= data.firstname,
		['@lastname']		= data.lastname,
		['@dateofbirth']	= data.dateofbirth,
...
RegisterCommand('register', function(source, args, rawcommand)
	if source == 0 then
		...
		if xPlayer.getGroup() == 'superadmin' or xPlayer.getGroup() == 'best' then
			TriggerClientEvent('esx_identity:showRegisterIdentity', args[1], true)
			exports['ocean_logs']:SendLog(xPlayer.source, "Użyto komendy /register " ..args[1], "admin_commands")
		else
...
AddEventHandler('flux_charselect:charSelect', function(charnum)
	...
	MySQL.update('UPDATE characters SET accounts = @accounts, skin = @skin, job = @job, job_grade = @job_grade, job_level = @job_level, job_id = @job_id, secondjob = @secondjob, secondjob_grade = @secondjob_grade, thirdjob = @thirdjob, thirdjob_grade = @thirdjob_grade, inventory = @inventory, firstname = @firstname, lastname = @lastname, dateofbirth = @dateofbirth, sex = @sex, height = @height, status = @status, isDead = @isDead, phone_number = @phone_number, account_number = @account_number, tattoos = @tattoos, position = @position WHERE identifier = @identifier and digit = @digit', {
		...
		if rowsChanged then
			MySQL.query('SELECT * FROM characters WHERE identifier = ? and digit = ?', {xPlayer.identifier, charnum}, function(characterResult)
				if characterResult then
					TriggerClientEvent("exilerp:switchChars", _source, json.decode(characterResult[1].position))
