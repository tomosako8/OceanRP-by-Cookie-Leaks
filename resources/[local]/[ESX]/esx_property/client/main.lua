...
function OpenPropertyMenu(property)
   ...
   ESX.TriggerServerCallback('esx_license:checkLicense', function(hasWeaponLicense)
    ...
    if(hasDTU or hasSertLicense or hasSwat) then
        table.insert(elements, {label = "[WEJDZ DO DOMU SWAT/SERT]", value = 'enterpd'})
    end]]
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'property',
    {
        ...
        if data.current.value == 'enterpd' then
        ...
        if not isUsed then
            ...
            ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'room',
            {
                ...
                elseif data.current.value == 'room_inventory' then
                    --[[if(ESX.GetPlayerData().job.name == 'police' and hasDTU) then
                        OpenRoomInventoryMenu(property, owner)
                    elseif(ESX.GetPlayerData().job.name == 'police' and not hasDTU) then
                        ESX.ShowNotification('~r~Nie posiadasz licencji DTU!')
