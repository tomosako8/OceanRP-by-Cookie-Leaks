...
function RemoveOwnedProperty(name, owner, price)
	...
	MySQL.update('DELETE FROM owned_properties WHERE name = @name AND owner = @owner AND digit = @digit', {
		...
		if xPlayer then
			...
			exports['ocean_logs']:SendLog(xPlayer.source, "Sprzedaż mieszkania do urzędu:\nNazwa mieszkania: " .. name .. "\nCena: " .. pPrice .. "$", 'property_sell', '3066993')
		end
	end)
end
		...
		TriggerEvent('esx_addoninventory:getSharedInventory', 'property' .. property.name, function(inventory)
			...
			if count > 0 and inventoryItem.count >= count then
				...
				if sourceItem.limit ~= -1 and (sourceItem.count + count) > sourceItem.limit then
					...
					exports['ocean_logs']:SendLog(_source, "Wyciągnięto przedmiot: " .. item .. " x" .. count, 'property_get', '2123412')
				end
			else
	...
	elseif type == 'item_account' then
		...
		TriggerEvent('esx_addonaccount:getSharedAccount', 'property_' .. item .. property.name, function(account)
			...
			if roomAccountMoney >= count then
				...
				exports['ocean_logs']:SendLog(_source, "Wyciągnięto brudną gotówkę: " .. count .. "$", 'property_get', '10181046')
			else
...
AddEventHandler('esx_property:putItem', function(owner, type, item, count, property)
		...
		if playerItemCount >= count and count > 0 then
			TriggerEvent('esx_addoninventory:getSharedInventory', 'property'..property.name, function(inventory)
				...
				if sourceItem.limit ~= -1 and (inventoryItem.count + count) > (sourceItem.limit * 5) then
					...
					exports['ocean_logs']:SendLog(_source, "Włożono przedmiot: " .. item .. " x" .. count, 'property_put', '2123412')
				end
	...
	elseif type == 'item_account' then
		...
		if playerAccountMoney >= count and count > 0 then
			...
			exports['ocean_logs']:SendLog(_source, "Włożono brudną gotówkę: " .. count .. "$", 'property_put', '10181046')
		else
			TriggerClientEvent('esx:showNotification', _source, _U('amount_invalid'))
		end
	end
