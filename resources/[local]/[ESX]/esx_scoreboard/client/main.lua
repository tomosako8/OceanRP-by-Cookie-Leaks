		...
		if Ped.Active and Ped.Exists then
			...
			if IsControlJustPressed(0, Keys['Z']) then
				...
				if IsEntityVisible(Ped.Id) then
					if not IsAdmin then
						TriggerServerEvent('esx_scoreboard:Show', "Przegląda listę obywateli ")
						if not IsPedInAnyVehicle(Ped.Id, false) and not exports["esx_ambulancejob"]:isDead() and not IsEntityDead(Ped.Id) and not IsPedFalling(Ped.Id) and not IsPedCuffed(Ped.Id) and not IsPedDiving(Ped.Id) and not IsPedInCover(Ped.Id, false) and not IsPedInParachuteFreeFall(Ped.Id) and GetPedParachuteState(Ped.Id) < 1 and not exports['ocean_trunk']:checkInTrunk() and not exports['esx_policejob']:IsCuffed() then
...
function PlayerList()
	...
	if Id and PlayerData.job then
		SendNUIMessage({
			...
			jobs   = {ems = MisiaczekPlayers['ambulance'], police = MisiaczekPlayers['police'], sheriff = MisiaczekPlayers['sheriff'], mechanik = MisiaczekPlayers['mechanik'], gheneraugarage = MisiaczekPlayers['gheneraugarage'], doj = MisiaczekPlayers['doj'], cardealer = MisiaczekPlayers['cardealer'], player_count = MisiaczekPlayers['players']}
		})
