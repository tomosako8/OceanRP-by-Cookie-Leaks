...
AddEventHandler('playerDropped', function()
	...
	if name ~= nil then
		...
		if xPlayer ~= nil then
			...
			MySQL.query('SELECT digit FROM db_clothes WHERE identifier = @identifier AND digit = @digit', {
				['@identifier'] = xPlayer.identifier,
				['@digit'] = digit
			}, function(result)
				if result[1] ~= nil then
					MySQL.update("UPDATE db_clothes SET bag=@bag, bag2=@bag2, tshirt=@tshirt , tshirt2=@tshirt2, torso=@torso, torso2=@torso2, legs=@legs, legs2=@legs2, shoes=@shoes, shoes2=@shoes2, arms=@arms, arms2=@arms2, chain=@chain, chain2=@chain2,mask=@mask,mask2=@mask2,decals=@decals,decals2=@decals2,hat=@hat,hat2=@hat2,watches=@watches,watches2=@watches2,bracelets=@bracelets,bracelets2=bracelets2,glasses=@glasses,glasses2=@glasses2,bproof1=@bproof1,bproof2=@bproof2, face=@face, hair=@hair WHERE identifier=@identifier AND digit = @digit", {
						['@identifier'] 	= xPlayer.identifier, 
						['@digit']		    = digit, 
						['@bag']		    = name.bags_1, 
						['@bag2'] 			= name.bags_2, 
