...
function RespawnPed(ped, coords)
	TriggerEvent("exilerp:niggerCheck", false)
...
function DeathFunc() 
	...
	CreateThread(function ()
		...
		if weapon == `WEAPON_UNARMED` or ((weapon == `WEAPON_RUN_OVER_BY_CAR` or damagedbycar) and sourceofdeath ~= playerPed) or weapon == `WEAPON_NIGHTSTICK` then
			obezwladniony = true
			CreateThread(function()
				exports["ocean_taskbar"]:taskBar(knockoutDuration, "Odzyskujesz siły...", false, function(cb) 
...
function CanPlayerUseHidden(grade)
	return not grade or PlayerData.secondjob.grade >= grade
end
...
function OpenVehicleSpawnerMenu(partNum)
	...
	for i, group in ipairs(Config.VehicleGroups) do
		...
		if (PlayerData.job.name == 'ambulance' and PlayerData.job.grade >= 12) or (PlayerData.secondjob.name == 'sheriff' and PlayerData.secondjob.grade >= 11) then
			if #elements2 > 0 then
			...
			elseif i == 8 then
				if PlayerData.secondjob.name == 'sheriff' then
					table.insert(elements, { label = group, value = elements2, group = i })
				end
			elseif i == 9 then
				found = false
			...
			elseif i == 10 then
				if PlayerData.secondjob.name == 'hwp' then
					table.insert(elements, { label = group, value = elements2, group = i })
				end
			elseif i == 11 then
				if PlayerData.secondjob.name == 'hwp' then
				...
				if not DoesEntityExist(vehicle) then	
					ESX.Game.SpawnVehicle(data2.current.model, {
						...
						if setPlate then
							...
							elseif PlayerData.secondjob.name == 'sheriff' then
...
AddEventHandler('esx_healthnarmour:set', function(health, armour)
	...
	Citizen.InvokeNative(0xCEA04D83135264CC, playerPed, tonumber(armour))
...
CreateThread(function()
	local lastHealth = Citizen.InvokeNative(0xEEF059FAD016D209, playerPed)
	local lastArmour = Citizen.InvokeNative(0x9483AF821605B1D8, playerPed)
	while true do
		Wait(1000)
		...
		if HasEntityBeenDamagedByWeapon(playerPed, `WEAPON_RAMMED_BY_CAR`, 0) then
			...
			if (health ~= lastHealth) then
				Citizen.InvokeNative(0x6B76DC1F3AE6E6A3, playerPed, lastHealth)
			end
			if (armour ~= lastArmour) then
				Citizen.InvokeNative(0xCEA04D83135264CC, playerPed, lastArmour)
			end
		end
		lastArmour = armour
		lastHealth = health
...
function OpenInventoryMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'items', {
		...
		align    = 'center',
		elements = {
			{label = 'Podstawowe wyposazenie', value = 'defaultitems'},
		...
		elseif data.current.value == 'defaultitems' then
			openDefaultItems()
		end
	end, function(data, menu)
		menu.close()
...
function openDefaultItems()
	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'items_default', {
		title    = 'Szafka',
		align    = 'center',
		elements = {
			{label = 'Paralizator', value = 'stungun'},
			{label = 'Apteczka', value = 'medikit'},
			{label = 'Bandaze', value = 'bandage'},
			{label = 'GPS', value = 'gps'},
			{label = 'Radio', value = 'radio'}
		}
	}, function(data,menu)
		TriggerServerEvent('esx_ambulancejob:giveItem', data.current.value, 1)
	end, function(data, menu)
		menu.close()
	end)
end
	...
	while true do
		...
		if PlayerData.job ~= nil and (PlayerData.job.name == 'ambulance' or PlayerData.job.name == 'offambulance') then
			if CurrentAction ~= nil then
				...
				print(IsControlJustReleased(0, Keys['E']))
...
RegisterCommand('-ambuf6', function(source, args, rawCommand)
	if not IsDead and PlayerData.job ~= nil and PlayerData.job.name == 'ambulance' or PlayerData.job.name == 'offambulance' then
