...
AddEventHandler('onResourceStart', function(resourceName)
	if resourceName == GetCurrentResourceName() then
		MySQL.query('SELECT * FROM holdup', function(result)
			for i=1, #result, 1 do
...
function UpdateDB(name, tTime)
	MySQL.update('UPDATE holdup SET nextRob = ? WHERE name = ?', {name,tTime})
end
...
AddEventHandler('esx_holdup:robberyStarted', function(currentStore)
	...
	if (cops['police']+cops['sheriff']) >= Config.typeNapad[store.type].cops then
		if LastRobbed[currentStore] == nil then 
			...
			MySQL.update('INSERT INTO holdup (name, nextRob) VALUES (@name, @nextRob)', 
			{
				['@name'] = currentStore,
				['@nextRob'] = 0
			})
		...
		if Stores[currentStore] then
			...
			if not rob then
				...
				for k,v in pairs(ESX.GetExtendedPlayers("job", "police")) do
					TriggerClientEvent('esx_holdup:setBlip', v.source, Stores[currentStore].position, _U('rob_in_prog', store.name))
				end
				for k,v in pairs(ESX.GetExtendedPlayers("job", "sheriff")) do
					TriggerClientEvent('esx_holdup:setBlip', v.source, Stores[currentStore].position, _U('rob_in_prog', store.name))
				end
				exports['ocean_logs']:SendLog(_source, "Rozpoczął napad na: "..currentStore, "napady")
					...
					if robbers[_source] then
						if xPlayer then
							...
							if rewardString ~= nil and rewardString ~= "" then
								...
								exports['ocean_logs']:SendLog(_source, "Zakończono napad na: "..currentStore.." Zarobek: "..store.reward .. rewardString, "napady")
							else
								TriggerClientEvent('esx_holdup:robberyComplete', _source, store.reward)
								exports['ocean_logs']:SendLog(_source, "Zakończono napad na: "..currentStore.." Zarobek: "..store.reward, "napady")
							end
							napad = false
