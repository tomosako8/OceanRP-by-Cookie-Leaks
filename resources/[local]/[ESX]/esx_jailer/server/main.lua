...
AddEventHandler('xlem0n_jailer:wpierdolchuja', function(target, jailTime)
	...
	MySQL.query('SELECT identifier FROM jail WHERE identifier=@id', {['@id'] = identifier}, function(result)
...
AddEventHandler('esx_jailer:reloadCharacter', function()
	...
	MySQL.query('SELECT timeleft, jail_time FROM jail WHERE identifier=@id AND digit = @digit', {['@id'] = xPlayer.identifier, ['@digit'] = xPlayer.getDigit()}, function(result)
		if result[1] ~= nil then
...
RegisterServerEvent("esx_jailer:triedToEscape", function(id) 
	...
	if chance <=15 then
		TriggerEvent('exilerp:triedToEscapePD', id)
	end
end)
