	...
	if DoesEntityExist(sourcePed) then
		...
		CreateThread(function()
			...
			while JailTime > 0 do
				...
				if #(GetEntityCoords(sourcePed) - vec3(JailLocation.x, JailLocation.y, JailLocation.z)) > 150.0 then
					...
					if not escapeCooldown then
						if chance <=5 then
							...
							TriggerEvent("chatMessage", '^0[^3Sędzia^0] ', { 147, 196, 109 }, "Z więzienia tak łatwo nie uciekniesz!")
						end
					else
						TriggerEvent("chatMessage", '^0[^3Sędzia^0] ', { 147, 196, 109 }, "Z więzienia tak łatwo nie uciekniesz!")
					end	
...
AddEventHandler('esx_jailer:escapeNotify', function(title, message)
	TriggerEvent("chatMessage", "^*"..title, { 61, 59, 59 }, message)
end)
